...
- `scripts/release/motion-lineage-gate.mjs --motion-root <clean-motion-root>
  --motion-commit <full-commit>` is the no-browser cross-repo release gate. It
  produces a real current-SDK handoff, maps it through Cut's Debug API, applies
  it through the MCP proxy, and requires the identical path-free proof in
  replayed project state. The gate owns an isolated HOME, server port, project,
  and scratch tree and removes all of them on success or failure.
...
- Connector code follows an enforced non-growth rule: package reads live in
  ...
  separate `motion_tracking/` modules. `scripts/module-size-gate.mjs` caps new
  feature modules at 350 lines/tests at 600 and locks reduced legacy ceilings.
