...
//! server verify.checks verb + render.final completion hook; e2e asserts.
...
/// duration_matches_edl: measured output duration == EDL duration, within
/// one frame at the project fps (rounded up).
pub fn duration_matches_edl(edl: &Edl, facts: &RenderFacts, fps: f64) -> CheckResult {
    ...
    CheckResult {
        ...
        pass: delta <= frame_ms,
        details: json!({"tolerance_ms": frame_ms, "fps": fps}),
        evidence: json!({
...
pub fn run_all_with_profile(
    ...
    let mut checks = match active {
        FootageProfile::TalkingHead => vec![
            ...
            duration_matches_edl(edl, facts, project.settings.fps),
        ],
...
mod tests {
    ...
    fn duration_check_one_frame_tolerance() {
        ...
        assert!(duration_matches_edl(&edl, &mk(5020), 30.0).pass);
        assert!(!duration_matches_edl(&edl, &mk(5100), 30.0).pass);
    }
