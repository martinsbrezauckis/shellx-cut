...
DEFAULT_CLI_MODEL = "sonnet"         # review workload, not authoring
...
def generate_render_perception(render: str, render_hash: str,
    ...
    if cp.returncode != 0 or not os.path.exists(out_path):
        raise RuntimeError(
            ...
            f"{process_failure_detail(cp.stdout, cp.stderr, 600)}")
    return out_path
...
# asserting these is outside its evidence — strip, never trust
# (docs/public/JUDGE_REVIEW.md §7.1.1).
#
...
def detect_frame_read_failure(review: dict, n_frames: int) -> tuple[bool, str | None]:
    ...
    cause = (
        ...
        f"judge's own words: \"{quote}\". A frame-read failure after preflight "
        f"is an infrastructure error rather than a completed review. "
...
def build_cli_prompts(mode: str, perception: dict, duration_s: float,
    ...
    # Execution-context addendum: tells the model how input is presented.
    sys_p += (
        "\nCLI EXECUTION CONTEXT: you run as a non-interactive subprocess with a"
...
def preflight_read_probe(claude_bin: str, model: str, bundle: str,
    ...
    try:
        ...
        return False, (
            "probe emitted non-JSON: "
            f"{process_failure_detail(cp.stdout, cp.stderr, 200)}")
    if env.get("is_error"):
        return False, (
