...
# model call. A present-but-logged-out agy reports found=true;
...
def detect() -> dict:
    ...
    if path:
        ...
        # No trustworthy auth-status command is available, so auth is
...
# gemini — plain absolute paths within the allowed bundle). The manifest is
# the same "frame N = ms" time map
...
def build_antigravity_prompts(mode: str, perception: dict, duration_s: float,
    ...
    and appends provider context so the model knows HOW it perceives
    ...
    # file tools open each frame within the --add-dir scope.
    ...
    sys_p += (
        "\nANTIGRAVITY CONTEXT: you run as a non-interactive `agy "
...
def invoke_antigravity(agy_bin: str, sys_p: str, user_p: str, model: str,
    ...
    Non-interactive Antigravity argv contract:
      --sandbox            terminal-restricted context. We use it INSTEAD of
      ...
      --print "<prompt>"   non-interactive; the prompt is the flag's VALUE and
                           MUST come LAST because a trailing flag is parsed as
                           part of the prompt value.
    ...
    Non-TTY mitigation: if `agy --print` produces empty stdout, the log file
    cannot recover a dropped response, so we return an honest error naming
...
def main() -> int:
    ...
    # agy opens frames from its --add-dir workspace through absolute paths in
    # the prompt, matching the CLI's documented workspace access model.
