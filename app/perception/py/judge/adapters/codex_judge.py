...
# properties in `required`; otherwise the CLI rejects the schema as non-strict.
...
def build_codex_prompts(mode: str, perception: dict, duration_s: float,
    ...
    sys_p += (
        "\nCODEX EXECUTION CONTEXT: you run as a non-interactive subprocess. The "
...
def invoke_codex(codex_bin: str, sys_p: str, user_p: str, model: str,
    ...
    no separate --system-prompt flag, so the execution rules ride in the same
...
def main() -> int:
    ...
    # /tmp, keeping every provider on one project-local bundle convention.
