...
# works), saving turns/quota. See the module docstring for the live evidence.
...
# model call, no quota burned. A present-but-logged-out grok reports found=true;
...
# ---------------------------------------------------------------------------
# Prompt assembly — judge.py templates + a grok harness note. grok reads frames
...
def build_grok_prompts(mode: str, perception: dict, duration_s: float,
    ...
    and appends a grok-harness note so the model knows HOW it perceives here:
    ...
    sys_p += (
        "\nGROK HARNESS CONTEXT: you run as a non-interactive `grok "
...
# (NB: flat data/mimeType — NOT Anthropic's nested source.{}; that shape is
# rejected "Invalid ACP content blocks: missing field `data`", verified live.)
# ---------------------------------------------------------------------------
...
def invoke_grok(grok_bin: str, sys_p: str, user_p: str, model: str,
    ...
    # value and die with OSError "Argument list too long" (Linux MAX_ARG_STRLEN
    # = 128 KB/arg) — VERIFIED live on colorful frames. The file channel has no
