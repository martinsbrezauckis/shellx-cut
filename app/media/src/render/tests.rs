...
/// the live proof). The hop states the input explicitly and targets the rec2020
...
/// authoritative proof is the Windows render. The prior escape-based attempt
/// passed its unit test yet still failed in real ffmpeg, so a string test alone
/// is necessary-but-not-sufficient (re-verified on the Windows build).
#[test]
...
fn eased_expr_matches_sample_via_ffmpeg() {
    ...
    eprintln!("easing live proof: 18 curves × 256 points, max |ffmpeg−sample| = {max_err}");
}
