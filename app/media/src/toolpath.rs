...
///
/// THE BUG THIS FIXES macOS): the auto-selector ranks purely by HW,
/// so a videotoolbox-but-libass-less Homebrew ffmpeg 8.x was chosen over a libass
/// build, and caption burn-in silently produced no caption. Caption/stabilize
...
fn doctor_ffmpeg_tool(stem: &str, prog: OsString) -> ToolStatus {
    ...
    let runnable = std::process::Command::new(&prog)
        .arg("-version")
