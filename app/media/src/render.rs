...
#[cfg(test)]
use stabilization::{
    stab_detect_filter_for_test as stab_detect_filter,
    stab_transform_filter_for_test as stab_transform_filter,
};
...
pub fn prepare_stabilization(
    ...
    stabilization::prepare(project, edl, project_dir)
}
            ...
            for (n, seg) in edl.track_segments(track_id).enumerate() {
                ...
                match (&seg.asset, seg.src_in_ms, seg.src_out_ms) {
                    (Some(asset), Some(src_in), Some(src_out)) => {
                        ...
                        let vlock = ramp_timing::video_suffix(project, seg);
                        // Speed retime (edit.speed): the setpts reset divides by the clip's speed so the trimmed source span plays in its
        ...
        for seg in edl.track_segments(track_id) {
            ...
            if let Some(am) = matte_alpha_idx {
                ...
                writeln!(
                    f,
                    "[{idx}:v]trim=start={trim_in}:end={trim_out},{vsetpts}{vstab}{vreverse}{vfreeze}{vcrop},\
                     {conform}{vanim}{vgrade}{veffects}{vcolor},{vfps},format=yuva420p{vopac}{place}{vfade}{vlock}[{label}];",
                    idx = input_idx[asset.as_str()],
                )
                .unwrap();
            }
        ...
        for (n, seg) in edl.track_segments(&track.id).enumerate() {
            ...
            match (&seg.asset, seg.src_in_ms, seg.src_out_ms) {
                (Some(asset), Some(src_in), Some(src_out)) => {
                    ...
                    let alock = ramp_timing::audio_suffix(project, seg);
                    ...
                    writeln!(
                        ...
                        "[{idx}:a]atrim=start={in_s}:end={out_s},asetpts=PTS-STARTPTS,{areverse}{aspeed}\
                         aformat=sample_fmts=fltp:sample_rates={rate}:channel_layouts=stereo{adenoise}{aeq}{gain}{amute}{afade}{alock}[{label}];",
                        idx = input_idx[asset],
                        in_s = secs(src_in),
                        out_s = secs(src_out),
                    )
...
/// in, the CUDA filter chain passes its runtime probe, the timeline is
...
fn render_gpu(
    ...
    run_progress_atomic(&args, out, edl.duration_ms, cb.as_ref())?;
...
pub fn render_final(
    ...
    let out = fence.fence_output_path(out_path)?; // the output-fencing contract — before any work
                                                  // Clear only an abandoned empty result before graph preparation. This also
                                                  // covers segmented renders that fail before their final mux reaches the
                                                  // atomic ffmpeg wrapper; a prior nonempty render remains available.
    crate::atomic_output::clear_stale_empty_output(&out)?;
...
/// by the SHARED GPU encoder + memory bandwidth, not RAM. Benchmarks showed that
/// fanning out to a RAM budget gave only **1.1–1.25× for 3–10× the RAM**
...
fn plan_seg_resources(
    window_ms: u64,
    gw: u32,
    gh: u32,
    overlays: usize,
) -> Result<(usize, WindowCaps), CutError> {
    ...
    Ok((parallelism, WindowCaps { high, max, threads }))
}
...
pub fn render_segmented(
    ...
    // Fan out independent capped windows; collect the first error after all scoped workers stop.
    let next = std::sync::atomic::AtomicUsize::new(0);
    let done = std::sync::atomic::AtomicUsize::new(0);
    let total_todo = todo.len().max(1);
    let errors: std::sync::Mutex<Vec<CutError>> = std::sync::Mutex::new(Vec::new());
    let process_control = crate::ffmpeg::current_render_process_control();
    std::thread::scope(|s| {
        for _ in 0..parallelism {
            s.spawn(|| {
                let _process_control =
                    crate::ffmpeg::scoped_render_process_control(process_control.as_ref());
    ...
    if let Some(aout) = agraph.audio_out.clone() {
        ...
        run_progress_atomic(&margs, out, dur, &|p| {
            cb(video_weight + p * (1.0 - video_weight))
        })?;
    } else {
        ...
        crate::ffmpeg::run_ffmpeg_atomic_output(&mv, out)?;
    }
    cb(1.0);
