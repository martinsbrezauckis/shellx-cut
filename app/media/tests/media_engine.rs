...
/// edit.reverse integration test: a clip that is GREEN for its first second then RED
...
/// edit.freeze integration test: a GREEN(0–1s)→RED(1–2s) clip frozen on a GREEN frame
...
/// edit.animate integration test: a GREEN-left | RED-right split frame, animated with a
...
/// edit.keyframe (opacity) integration test: a full-frame GREEN overlay over a RED base,
...
/// edit.keyframe POSITION (pos_x) integration test: a small green PiP slides left→right
...
/// edit.eq integration test: a voice-shape EQ (high-pass 120 Hz, +6 dB @1 kHz presence,
...
/// edit.effect Gate integration test: a noise gate (agate) PASSES a loud section and
...
/// Opt-in stabilization quality test. It renders the same synthetic shake with
/// and without the real two-pass vidstab pipeline and requires less inter-frame
/// motion. It is ignored by default because filter-thread scheduling can vary
/// across ffmpeg builds; run it explicitly when validating a local ffmpeg.
#[test]
#[ignore = "vidstab output varies across ffmpeg builds; run explicitly"]
...
/// edit.blend LAYER blend mode integration test: a green PiP on an overlay track set to
...
#[test]
fn corrupt_cached_proxy_is_rebuilt_before_reuse() {
    let dir = tempfile::tempdir().unwrap();
    let clip = gen_clip(dir.path(), "in.mp4");
    let proxies = dir.path().join("proxies");
    std::fs::create_dir_all(&proxies).unwrap();
    ...
    std::fs::write(&proxy, b"nonempty partial MP4 with no moov atom").unwrap();

    let rebuilt = make_proxy(&clip, &proxies, "a1")
        .expect("an invalid cached proxy must be replaced by a completed proxy");
    let info = probe(&rebuilt).expect("replacement proxy must be ffprobe-valid");

    assert_eq!(rebuilt, proxy);
    assert_eq!((info.width, info.height), (Some(960), Some(540)));
    assert!(info.has_audio);
...
/// On supported hardware this is the end-to-end proof that the GPU graph renders.
#[test]
...
/// Optional real-4K software integration test. It requires a caller-provided 4K
/// clip and output directory and writes `real4k_sw.mp4`.
#[test]
...
/// Optional real-4K GPU integration test. Run with `SHELLX_CUT_RENDER_GPU=1`
/// and a CUDA-capable `SHELLX_CUT_FFMPEG`. It requires the GPU pipeline to engage
/// and writes `real4k_gpu.mp4` for comparison with the software output.
...
fn hw_encoder_declines_frame_sizes_it_cannot_encode() {
    ...
    assert!(
        checked > 0,
        "no hardware encoder available — run this test on a GPU machine"
    );
}
