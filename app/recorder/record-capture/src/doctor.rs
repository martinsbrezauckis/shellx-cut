...
//! backend, the input hook, and webcam — each with status ok/missing/degraded and
...
/// Parse `gdbus call … org.gnome.ScreenSaver.GetActive` output (e.g. `(true,)` / `(false,)`).
/// `Some(true)` = screensaver/lock active, `Some(false)` = inactive, `None` = unknown (so a
/// gdbus error or unexpected text never false-degrades). Pure → unit-testable.
...
fn session_locked() -> Option<bool> {
    ...
    if let Ok(sid) = std::env::var("XDG_SESSION_ID") {
        if let Ok(o) = Command::new("loginctl")
            .args(["show-session", &sid, "-p", "LockedHint"])
            .output()
        {
            if o.status.success() {
...
pub fn doctor() -> Vec<Card> {
    ...
    let (sc_status, sc_detail) = screen_backend();
    cards.push(Card::new("screen_capture", "capture", sc_status, sc_detail));
    ...
    #[cfg(all(target_os = "linux", feature = "capture-linux"))]
    {
        ...
        let has_gst = Command::new(&gst)
            .arg("--version")
            .output()
            .map(|o| o.status.success())
            .unwrap_or(false);
        let has_pw = Command::new("gst-inspect-1.0")
            .arg("pipewiresrc")
            .output()
            .map(|o| o.status.success())
            .unwrap_or(false);
...
mod tests {
    ...
    fn doctor_reports_all_cards() {
        ...
        for c in &cards {
            assert!(["ok", "missing", "degraded"].contains(&c.status.as_str()));
