...
struct TapCtx {
    ...
    uint64_t first_packet_ms = std::numeric_limits<uint64_t>::max();
};
...
OSStatus tap_ioproc(AudioObjectID inDevice, const AudioTimeStamp* inNow,
    ...
    if (bl->mNumberBuffers >= 2 && bl->mBuffers[0].mNumberChannels == 1) {
        ...
        if (L && R) {
            ...
            if (frames && c->first_packet_ms == std::numeric_limits<uint64_t>::max()) {
                c->first_packet_ms = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
        ...
        if (b->mData) {
            ...
            if (n && c->first_packet_ms == std::numeric_limits<uint64_t>::max()) {
                c->first_packet_ms = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::steady_clock::now() - c->started).count());
            }
...
extern "C" int sxc_sysaudio_stop(void* h, float** out_samples, uint64_t* out_count,
                                 uint32_t* out_channels, double* out_rate, uint64_t* out_first_packet_ms) {
    if (out_samples) *out_samples = nullptr;
    if (out_count) *out_count = 0;
    if (out_channels) *out_channels = 0;
    if (out_rate) *out_rate = 0.0;
    if (out_first_packet_ms) *out_first_packet_ms = std::numeric_limits<uint64_t>::max();
    if (!h) return -1;
    ...
    uint64_t first_packet_ms = std::numeric_limits<uint64_t>::max();
    {
        ...
        first_packet_ms = c->first_packet_ms;
    }
    delete c;
    ...
    *out_rate = rate;
    if (out_first_packet_ms) *out_first_packet_ms = first_packet_ms;
    if (!n) return 0;
