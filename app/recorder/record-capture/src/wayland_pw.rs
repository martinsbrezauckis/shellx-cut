...
//!
//! THE CRITICAL DETAIL (cost a long debug): mutter offers the cursor meta at a FIXED
...
//! Verified live on GNOME 46 Wayland (RTX 5080): pixel-accurate, hits the 4K corner.
//!
...
/// encode frames to `raw_path` (.mp4, variable PTS) and return the absolute cursor
/// track (sampled from SPA_META_Cursor, timestamped on the shared `start` clock so it
/// aligns with the evdev click/key track). Stops early if `stop` is set.
///
/// Returns the cursor samples; the video is left at `raw_path` for the caller's
/// CFR-normalize pass. Runs the PipeWire main loop on the calling thread (blocking).
pub fn capture(
    ...
    raw_path: &str,
    ff_bin: &str,
) -> Result<Vec<CursorSample>, String> {
    ...
    // Portal path: connect over the restricted remote fd. Direct-mutter test path
            ...
            if let Some(cursor) = buffer.find_meta::<spa::buffer::meta::MetaCursor>() {
                if cursor.id() != 0 {
                    ...
                    if st.last != Some((x, y)) {
                        st.last = Some((x, y));
                        let t_ms =
                            u64::try_from(st.start.elapsed().as_millis()).unwrap_or(u64::MAX);
                        st.cursor.push(CursorSample {
                            t_ms,
                            x: f64::from(x),
                            y: f64::from(y),
                        });
    ...
    let (stdin, child, cursor, pixel_frames) = {
        let mut st = state.borrow_mut();
        (
            st.ff_stdin.take(),
            st.ff_child.take(),
            std::mem::take(&mut st.cursor),
            st.pixel_frames,
        )
    };
