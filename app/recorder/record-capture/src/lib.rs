...
// pub(doc-hidden): the capture fn is callable from a direct-mutter self-test harness
// (no portal consent) to exercise this exact code path without a desktop dialog.
#[cfg(all(target_os = "linux", feature = "capture-linux"))]
#[doc(hidden)]
pub mod wayland_pw;
...
/// audio) → 16-bit WAV (no ffmpeg, no virtual cable).
