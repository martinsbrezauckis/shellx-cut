...
!macro NSIS_HOOK_PREINSTALL
  nsExec::ExecToStack 'powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "& { $$target = [IO.Path]::GetFullPath($\"$INSTDIR\cutd.exe$\"); $$owned = @(Get-CimInstance Win32_Process -Filter $\"Name=''cutd.exe''$\" -ErrorAction Stop | Where-Object { $$_.ExecutablePath -and [IO.Path]::GetFullPath($$_.ExecutablePath).Equals($$target, [StringComparison]::OrdinalIgnoreCase) }); foreach ($$process in $$owned) { Stop-Process -Id $$process.ProcessId -Force -ErrorAction Stop }; if ($$owned.Count -gt 0) { Start-Sleep -Milliseconds 500 }; $$remaining = @(Get-CimInstance Win32_Process -Filter $\"Name=''cutd.exe''$\" -ErrorAction Stop | Where-Object { $$_.ExecutablePath -and [IO.Path]::GetFullPath($$_.ExecutablePath).Equals($$target, [StringComparison]::OrdinalIgnoreCase) }); if ($$remaining.Count -ne 0) { exit 42 } }"'
  Pop $0
  Pop $1
