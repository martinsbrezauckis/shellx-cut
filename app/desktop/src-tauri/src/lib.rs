...
/// Narrow opt-in used by the installed Windows qualification harness. Wry
...
fn spawn_engine(
...
) -> Result<(Option<Child>, String, bool), String> {
    let (addr, reuse) = pick_engine_addr()?;
    let url = format!("http://{addr}/");
    if reuse {
        let ui_present = cutd_serves_ui(&addr);
        eprintln!("[shellx-cut] reusing external cutd at {url} (ui: {ui_present})");
        return Ok((None, url, ui_present));
    }
    ...
    loop {
        if is_cutd(&addr) {
            eprintln!("[shellx-cut] engine ready at {url} (ui: {ui_present})");
            return Ok((Some(child), url, ui_present));
        }
...
pub fn run() {
    ...
    let webview_test_data_directory = {
        ...
        let data_directory = data_token.map(|token| {
            ...
            local_app_data.join("ShellX Cut Qualification").join(token)
        });
    ...
    // drive macOS WKWebView. Shipping build scripts reject this feature.
    #[cfg(feature = "webdriver-test")]
    let builder = {
        // Keep an unambiguous marker in instrumented binaries. Installed Linux
        // qualification uses an external driver whose own native port is open,
    ...
    let builder = builder
        ...
        .setup(move |app| {
            ...
            // production proof of WHICH tools the installed app resolved.
            // tools_doctor remains a local-origin verification/e2e surface.
                    ...
                    match capability {
                        Ok(()) => {
                            ...
                            *app.state::<EngineStatus>().0.lock().unwrap() =
                                EngineState::Wired { url: url.clone(), mode, ui, tools_hint };
