...
//!   only windows-x86_64 + darwin-aarch64 (see the 2026-08-06 demo-shoot
//!   finding). On Linux the snapshot reports `status: "unsupported"` and no
//!   network request is ever made; the UI shows the honest explanation instead
//!   of dead buttons.
//!
//! SECURITY
//!   The updater plugin verifies the minisign signature against the bundled
//!   `plugins.updater.pubkey` before installing, and the version comparator in
//!   lib.rs additionally requires release URLs to be bound to the manifest
//!   version — both apply unchanged to every check this service performs.
//!   `SHELLX_CUT_UPDATE_FEED_URL` (QA staging: point a test install at a
...
impl Snapshot {
    ...
    ///
    /// Rules (the red-proofed state machine):
...
async fn install_pending(app: &tauri::AppHandle) -> Result<serde_json::Value, String> {
    ...
    // handles the restart; elsewhere this triggers it). Never returns.
    app.restart();
}

// ─────────────────────────────────────────────────────────────────────────────
// Red-proof tests for the pure state machine.
// ─────────────────────────────────────────────────────────────────────────────
