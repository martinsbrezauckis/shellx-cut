...
//! funnels into dispatch() — same envelope as REST (coverage audit: REST and
//! MCP both expose 100% of verbs by construction).
//!
...
pub async fn run_stdio(state: AppState, mode: McpMode) -> anyhow::Result<()> {
    ...
    for line in stdin.lock().lines() {
        ...
        let reply = match method {
            ...
            "tools/list" => rpc_result(id, json!({"tools": list_tools(&state)})),
            "tools/call" => {
                ...
                match state.registry.verb_for_tool(tool) {
                    ...
                    Some(spec) => {
                        ...
                        let envelope_json = match mode {
                            McpMode::Proxy => match crate::httpc::post_verb(&verb_name, &args) {
                                Ok(v) => v,
