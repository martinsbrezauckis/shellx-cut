...
/// FIRST, then the explicit agent install dirs incl. grok's off-PATH ~/.grok/bin and
/// a Finder-stripped-PATH .app's Homebrew/npm dirs) via [`resolve_agent`] — NOT a
/// process-PATH-only scan. This fixes the same off-PATH-grok bug the chat path fixed:
/// a grok that lives only in ~/.grok/bin now DETECTS (and `assets_generate` then
/// spawns the resolved absolute path). The on-PATH case is byte-identical to the old
/// PATH-only check because `resolve_agent` searches the process PATH first. Mirrors
...
//
// THE BUG THIS FIXES: a process-PATH-only check (what `detect` used to do) only
// consults the PROCESS PATH. Two real
// installs break that:
//   * grok self-manages a symlink at ~/.grok/bin/grok that it NEVER adds to PATH,
//     so an on-PATH-only check reports grok absent even when it runs fine
//     (confirmed on macOS: ~/.grok/bin/grok can work while remaining off PATH).
