...
/// registry asset is `<BASE>/<asset>`; the checksum file is
/// `<BASE>/checksums.sha256` (verified live to be the ONLY checksum surface —
/// no per-asset .sha256 files, GitHub API.
...
fn extract_zip(archive: &Path, dest: &Path) -> Result<(), CutError> {
    ...
    let tar_try = std::process::Command::new("tar")
        .arg("-xf")
        .arg(archive)
        .arg("-C")
        .arg(dest)
        ...
        .status();
    if matches!(tar_try, Ok(s) if s.success()) {
        return Ok(());
    }
    let status = std::process::Command::new("powershell")
        .args(powershell_expand_archive_args(archive, dest))
        .stdin(std::process::Stdio::null())
        .status()
        .map_err(|e| {
            CutError::new(
    ...
    if status.success() {
        ...
        Err(CutError::new(
            ...
            format!("Expand-Archive exited {status}"),
        ))
    }
}
...
fn run_extract(prog: &str, args: &[&std::ffi::OsStr], archive: &Path) -> Result<(), CutError> {
    ...
    if status.success() {
        ...
        Err(CutError::new(
            ...
            format!("'{prog}' exited {status} on {}", archive.display()),
        ))
    }
}
