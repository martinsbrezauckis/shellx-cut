...
fn probe_exec(prog: &std::ffi::OsStr, args: &[&str], timeout: Duration) -> ProbeOutcome {
    ...
    {
        Ok(c) => c,
        // A failed spawn is the ONLY confirmed-absence signal; everything past
        // here means the process DID start (so present, however slow).
        Err(_) => return ProbeOutcome::NotFound,
    };

    // Poll for completion up to `timeout`; kill on overrun ⇒ Timeout (unverified).
    ...
    loop {
        match child.try_wait() {
            ...
            // A wait error is NOT a confirmed absence — treat it as unverified.
...
/// → authenticated (best-effort) → ready, plus the informational posture tag.
fn chat_agent_block(provider: &str, found: bool, resolved: Option<&Path>) -> Value {
    if !crate::chat::CHAT_AGENTS.contains(&provider) {
        return Value::Null;
    }
    let wired = crate::chat::is_wired(provider);
    let (authenticated, auth_detail) = chat_auth_state(provider, resolved);
    // READY = installed && wired && a CONFIRMED auth ("yes"). "unknown" (grok's
    // file-present-but-unverifiable case) is deliberately NOT ready — the dropdown
    // can still offer it, but flagged "auth not verified" rather than a false green.
    ...
    json!({
        ...
        "ready": ready,
        "posture": crate::chat::security_posture(provider),
    })
}
        ...
        .map(|&(provider, bin, vflag)| {
            ...
            Card {
                ...
                details: json!({
                    ...
                    "chat": chat_agent_block(provider, found, resolved.as_deref()),
                }),
            }
        })
        .collect()
...
mod tests {
    ...
    fn judge_cards_carry_chat_agent_state_and_posture() {
        ...
        // informational posture badge — for the three chat agents only.
        let cards = judge_cards();
        for (id, posture) in [
            ("judge.claude", "editor-sandboxed"),
            ("judge.codex", "full system access"),
            ("judge.grok", "cutd-tools"),
        ] {
            ...
            assert_eq!(chat["wired"], serde_json::json!(true), "{id} is chat-wired");
