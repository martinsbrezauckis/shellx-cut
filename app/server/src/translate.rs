    ...
    match backend {
        ...
        Backend::Local => {
            ...
            let mut child = command.spawn().map_err(|e| {
                CutError::new(
                    error_codes::IO,
                    format!("could not start the local translate runner: {e}"),
                    "the perception venv python + translate_runner.py must exist",
                )
            })?;
            ...
            if let Some(mut stdin) = child.stdin.take() {
                if let Err(e) = stdin.write_all(stdin_payload.as_bytes()).await {
                    ...
                    return Err(CutError::new(
                        error_codes::IO,
                        format!("local translate runner stdin write failed: {e}"),
                        "local MT failed before the runner received its request",
                    ));
                }
                if let Err(e) = stdin.shutdown().await {
                    ...
                    return Err(CutError::new(
                        error_codes::IO,
                        format!("local translate runner stdin close failed: {e}"),
                        "local MT failed before the runner received a complete request",
                    ));
                }
            } else {
                let _ = child.kill().await;
                return Err(CutError::new(
                    error_codes::IO,
                    "local translate runner stdin pipe unavailable",
                    "local MT failed before the runner received its request",
                ));
            }
            ...
            let out = match waited {
                ...
                Ok(Err(e)) => {
                    return Err(CutError::new(
                        error_codes::IO,
                        format!("the local translate runner errored: {e}"),
                        "local MT failed",
                    ))
                }
                Err(_) => {
...
async fn run_translation_cli_agent(
    ...
    let mut child = command.spawn().map_err(|e| {
        ...
        CutError::new(
            ...
            "the translation CLI must be installed + logged in + on PATH",
        )
        .with_suggested_action("install/sign-in to claude, or use backend:\"local\"")
    })?;
    let mut stdin_error = None;
    if cmd.via_stdin {
        if let Some(mut stdin) = child.stdin.take() {
            if let Err(e) = stdin.write_all(prompt.as_bytes()).await {
                // The CLI may have exited early after writing a useful auth,
                // quota, or configuration error. Keep collecting its output
                // instead of replacing that diagnostic with BrokenPipe.
                stdin_error = Some(
                    CutError::new(
                        error_codes::IO,
                        format!("translation CLI stdin write failed: {e}"),
                        "translation CLI failed before the agent received its request",
                    )
                    .with_suggested_action(
                        "confirm the translation CLI can read prompts from stdin",
                    ),
                );
                ...
                stdin_error = Some(
                    CutError::new(
                        error_codes::IO,
                        format!("translation CLI stdin close failed: {e}"),
                        "translation CLI failed before the agent received a complete request",
                    )
                    .with_suggested_action(
                        "confirm the translation CLI can read prompts from stdin",
                    ),
            ...
            return Err(CutError::new(
                error_codes::IO,
                "translation CLI stdin pipe unavailable",
                "translation CLI failed before the agent received its request",
            )
            .with_suggested_action("confirm the translation CLI can read prompts from stdin"));
        }
    }
    let waited = tokio::time::timeout(timeout, child.wait_with_output()).await;
    let _ = std::fs::remove_dir_all(&ws);
    let out = match waited {
        ...
        Ok(Err(e)) => {
            return Err(CutError::new(
                error_codes::IO,
                format!("the {} CLI errored: {e}", cmd.cmd),
...
mod tests {
    ...
    fn parses_flat_string_array() {
        // exactly what the claude CLI returned live
