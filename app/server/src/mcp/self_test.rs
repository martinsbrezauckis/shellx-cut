...
pub(crate) async fn run(state: &AppState) -> Result<VerbResult, CutError> {
    ...
    let mut child = tokio::process::Command::new(&executable)
        ...
        .map_err(|error| {
            test_error(
                "could not start the installed Cut MCP executable",
                &error.to_string(),
            )
        })?;
    let mut stdin = child.stdin.take().ok_or_else(|| {
        test_error(
            "could not open the MCP self-test input stream",
            "child process stdin was not piped",
        )
    })?;
    stdin.write_all(input.as_bytes()).await.map_err(|error| {
        test_error(
            "could not write the MCP self-test requests",
            &error.to_string(),
        )
    })?;
    drop(stdin);
    ...
    let output = tokio::time::timeout(Duration::from_secs(12), child.wait_with_output())
        ...
        .map_err(|error| {
            test_error(
                "could not collect the MCP self-test result",
                &error.to_string(),
            )
        })?;
