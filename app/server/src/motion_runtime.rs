...
pub(crate) async fn run_motion_command_spec(
    ...
    let mut child = command.spawn().map_err(|error| {
        CutError::new(
            error_codes::SIDECAR,
            format!("ShellX Motion CLI could not {operation}"),
            format!("{}: {error}", spec.program),
        )
    })?;
    let mut stdout = child.stdout.take().expect("piped Motion stdout");
    let mut stderr = child.stderr.take().expect("piped Motion stderr");
    let capture = async move {
        let stdout_task = async {
            let mut bytes = Vec::new();
            (&mut stdout)
                .take(MOTION_COMMAND_OUTPUT_MAX_BYTES + 1)
    ...
    let (stdout, stderr, status) =
        tokio::time::timeout(Duration::from_millis(spec.timeout_ms), capture)
            .await
            .map_err(|_| {
                CutError::new(
                    error_codes::SIDECAR,
    ...
    if stdout.len() as u64 > MOTION_COMMAND_OUTPUT_MAX_BYTES
        || stderr.len() as u64 > MOTION_COMMAND_OUTPUT_MAX_BYTES
    {
    ...
    parse_motion_process_output(&stdout, &stderr, status.success(), operation)
}
