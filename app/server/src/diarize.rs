...
pub async fn run_diarize(
    ...
    let mut child = command.spawn().map_err(|e| {
        CutError::new(
            error_codes::IO,
            format!("could not start the diarize runner: {e}"),
            "the perception python + diarize_runner.py must exist",
        )
        .with_suggested_action(
            "install the perception sidecar, or set DIARIZE_RUNNER_PY / DIARIZE_RUNNER_SCRIPT",
        )
    })?;
    if let Some(mut stdin) = child.stdin.take() {
        if let Err(e) = stdin.write_all(payload.as_bytes()).await {
            ...
            return Err(CutError::new(
                error_codes::IO,
                format!("diarize runner stdin write failed: {e}"),
                "speaker diarization failed before the runner received its request",
            ));
        }
        if let Err(e) = stdin.shutdown().await {
            ...
            return Err(CutError::new(
                error_codes::IO,
                format!("diarize runner stdin close failed: {e}"),
                "speaker diarization failed before the runner received a complete request",
            ));
        }
    } else {
        let _ = child.kill().await;
        return Err(CutError::new(
            error_codes::IO,
            "diarize runner stdin pipe unavailable",
            "speaker diarization failed before the runner received its request",
        ));
    }
    ...
    let out = match waited {
        ...
        Ok(Err(e)) => {
            return Err(CutError::new(
                error_codes::IO,
                format!("the diarize runner errored: {e}"),
