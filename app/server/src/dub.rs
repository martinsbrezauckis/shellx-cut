...
pub(crate) async fn audio_dub(
    ...
    let rationale = args
        ...
        .and_then(|r| r.as_str())
        .map(String::from);
    let include_legacy_inverse = crate::dispatch::wants_legacy_inverse(&args);
    ...
    Ok(VerbResult::ok_with_ops(
        json!({
            ...
            "receipt": rel,
            "clips": clips,
            "op": crate::dispatch::op_for_result(&op, include_legacy_inverse),
        }),
        vec![op_id],
    )
...
pub async fn synthesize_track(
    ...
    let out = match run_owned(&mut command, Some(payload.as_bytes()), &control).await {
        ...
        Err(error) => match error.termination() {
            ...
            Some(ProcessTermination::Cancelled(reason)) => {
                return Err(CutError::new(
                    "job_cancelled",
                    format!("dubbing synthesis cancelled ({})", reason.label()),
            ...
            None => {
                return Err(CutError::new(
                    error_codes::IO,
                    format!("the dub runner errored: {error}"),
    ...
    if out.diagnostics_truncated() {
        tracing::warn!("dub runner diagnostics exceeded the retained output cap");
    }
