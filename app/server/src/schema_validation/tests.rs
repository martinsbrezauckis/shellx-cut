use super::*;
use crate::registry::{verb_contract::DispatchTarget, VerbBehavior, VerbRegistry};
use cut_core::{
    AgentChatCapability, AsyncJobType, Idempotency, MutationClass, ProjectState, Replayability,
    SideEffects, UiExposure, VerbRisk,
};
...
const SUPPORTED_INPUT_KEYWORDS: &[&str] = &[
    ...
    "const",
    "default",
    // JSON Schema annotation: it is intentionally transparent to validation but
    // must remain visible in the executable public contract.
    "deprecated",
    "description",
    "enum",
    "exclusiveMaximum",
...
fn spec(name: &str, args: Value) -> VerbSpec {
    VerbSpec {
        ...
        behavior: VerbBehavior {
            ...
            dispatch: DispatchTarget::ProjectCreate,
            project_state: ProjectState::None,
            idempotency: Idempotency::NotApplicable,
            replayability: Replayability::NotApplicable,
            async_job: AsyncJobType::None,
            ui_exposure: UiExposure::Internal,
            agent_chat: AgentChatCapability::Deny,
            risk: VerbRisk::None,
            facets: vec![],
        },
        result_schema: None,
