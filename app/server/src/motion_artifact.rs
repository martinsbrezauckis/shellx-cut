...
async fn probe_media_bounded(path: &Path, media_type: &str) -> Result<(), CutError> {
    ...
    let mut child = command.spawn().map_err(|error| {
        artifact_error(
            "failed to run ffprobe for Motion artifact",
            error.to_string(),
        )
    })?;
    let stdout = child.stdout.take().ok_or_else(|| {
        artifact_error(
            "Motion artifact ffprobe stdout was not captured",
            "internal process error",
        )
    })?;
    let stderr = child.stderr.take().ok_or_else(|| {
        artifact_error(
            "Motion artifact ffprobe stderr was not captured",
            "internal process error",
        )
    })?;
    ...
    let (status, stdout, stderr) = tokio::time::timeout(FFPROBE_TIMEOUT, communicate)
        .await
        .map_err(|_| {
            artifact_error(
    ...
    if stdout.len() > FFPROBE_MAX_OUTPUT_BYTES || stderr.len() > FFPROBE_MAX_OUTPUT_BYTES {
        return Err(artifact_error(
    ...
    let probe: Value = serde_json::from_slice(&stdout).map_err(|error| {
        artifact_error(
...
async fn read_probe_stream(reader: impl AsyncRead + Unpin) -> std::io::Result<Vec<u8>> {
    let mut bytes = Vec::new();
    reader
        .take((FFPROBE_MAX_OUTPUT_BYTES + 1) as u64)
