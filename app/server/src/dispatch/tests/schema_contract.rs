...
async fn legacy_inverse_compatibility_option_is_deprecated_and_type_checked() {
    ...
    assert_eq!(
        spec.args["properties"]["include_inverse"]["deprecated"], true,
        "the v0.6 compatibility option must be visibly deprecated"
    );
    let compatibility_verbs: Vec<_> = state
        .registry
        .verbs
        .iter()
        .filter(|verb| verb.args["properties"].get("include_inverse").is_some())
        .collect();
    assert_eq!(
        compatibility_verbs.len(),
        67,
        "every retained include_inverse surface is intentional"
    );
    for verb in compatibility_verbs {
        let option = &verb.args["properties"]["include_inverse"];
        assert_eq!(option["type"], "boolean", "{}", verb.name);
        assert_eq!(option["deprecated"], true, "{}", verb.name);
    }
    assert_schema_error(
        dispatch(
            &state,
