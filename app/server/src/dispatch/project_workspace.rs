...
pub(super) async fn project_create(state: &AppState, args: Value) -> Result<VerbResult, CutError> {
    ...
    let store = ProjectStore::create(&parent, &a.name, a.settings)?;
...
pub(super) async fn project_open(state: &AppState, args: Value) -> Result<VerbResult, CutError> {
    ...
    let payload = json!({"path": store.dir, "project": store.project});
...
pub(super) async fn project_state(state: &AppState) -> Result<VerbResult, CutError> {
    let guard = state.project.read().await;
    let store = guard.as_ref().ok_or_else(no_project)?;
...
pub(super) async fn project_ops(state: &AppState, args: Value) -> Result<VerbResult, CutError> {
    ...
    let guard = state.project.read().await;
    let store = guard.as_ref().ok_or_else(no_project)?;
    // Resolve a checkpoint ref → its op id; a raw op id passes through. Unknown
    // refs surface resolve_ref's "neither a checkpoint nor an op id" error
    // (more actionable than read_since's raw-op-id-only "not found").
    let since_op = match a.since.as_deref() {
        None => None,
        Some(r) => Some(cut_core::diff::resolve_ref(&store.project, &r.to_string())?),
    };
    let ops = store.log.read_since(since_op.as_deref())?;
    // Drive the UI Undo/Redo button enabled-state + keyboard handlers off the
    // live linear-history cursor. Additive fields; existing readers of
    // {ops} are unaffected.
    Ok(VerbResult::ok(json!({
        "ops": ops,
...
async fn run_draft_adapter(adapter: Option<&Path>, context: &Value) -> Value {
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            return json!({"status": "error", "reason": format!("draft adapter spawn failed: {e}"), "draft": Value::Null})
        }
    };
    // Write the request to the child's stdin, then drop it (EOF).
    if let Some(mut stdin) = child.stdin.take() {
        use tokio::io::AsyncWriteExt;
        if let Err(e) = stdin.write_all(input.as_bytes()).await {
            let _ = child.kill().await;
            return json!({"status": "error", "reason": format!("draft adapter stdin write failed: {e}"), "draft": Value::Null});
        }
        if let Err(e) = stdin.shutdown().await {
            let _ = child.kill().await;
            return json!({"status": "error", "reason": format!("draft adapter stdin close failed: {e}"), "draft": Value::Null});
        }
    } else {
        let _ = child.kill().await;
        return json!({"status": "error", "reason": "draft adapter stdin pipe was unavailable", "draft": Value::Null});
    }
    let out = match tokio::time::timeout(
        std::time::Duration::from_secs(DRAFT_TIMEOUT_S),
    ...
    .await
    {
        Ok(Ok(o)) => o,
        Ok(Err(e)) => {
            return json!({"status": "error", "reason": format!("draft adapter io error: {e}"), "draft": Value::Null})
        }
        Err(_) => {
