...
pub(super) async fn comment_export(state: &AppState, args: Value) -> Result<VerbResult, CutError> {
    ...
    let html_path = fence_project_output_path(&dir, a.path.as_deref(), &default)?;
    if html_path
        .extension()
        .and_then(|x| x.to_str())
        .is_none_or(|x| !x.eq_ignore_ascii_case("html"))
    {
        return Err(CutError::new(
            error_codes::INVALID_ARGS,
            "review package path must end in .html",
    ...
    let media_path = fence_output_path(&dir, Some(&media_path.display().to_string()), "unused")?;
    let manifest_path =
        fence_output_path(&dir, Some(&manifest_path.display().to_string()), "unused")?;
