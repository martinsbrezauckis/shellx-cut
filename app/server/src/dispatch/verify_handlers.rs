...
fn judge_intent(
    ...
    let ops = store.log.read_since(since.as_deref()).unwrap_or_default();
    let mut lines: Vec<String> = ops
        .iter()
        .filter(|o| o.op_id.as_str() <= receipt.at_op.as_str())
...
/// reports its timeout itself where possible). stderr is captured into the
/// failure cause (actionable, not swallowed).
async fn run_judge_adapter(
    ...
    cmd.arg(adapter)
        ...
        .kill_on_drop(true); // outer-timeout drop kills the child
    ...
    let spawned = match cmd.spawn() {
        ...
        Err(e) => {
            let cause = format!("judge adapter spawn failed: {e}");
            return JudgeRun {
                envelope: synth_judge_envelope("error", cause.clone()),
                status: "error".into(),
                fail_cause: Some(cause),
    ...
    let output = match tokio::time::timeout(
        std::time::Duration::from_secs(outer_s),
    ...
    .await
    {
        Err(_elapsed) => {
            let cause = format!(
                "judge adapter exceeded the {outer_s}s wall-clock timeout (CUTD_JUDGE_TIMEOUT_S) and was killed"
            );
            return JudgeRun {
                envelope: synth_judge_envelope("error", cause.clone()),
                status: "error".into(),
        ...
        Ok(Err(e)) => {
            let cause = format!("judge adapter I/O failed: {e}");
            return JudgeRun {
                envelope: synth_judge_envelope("error", cause.clone()),
                status: "error".into(),
                fail_cause: Some(cause),
    ...
    let stderr_tail = |max: usize| -> String {
        let s = String::from_utf8_lossy(&output.stderr);
        let t = s.trim();
        t.chars()
            .skip(t.chars().count().saturating_sub(max))
            .collect()
    };
    if !output.status.success() {
        // Exit 2 = the adapter refused the input (e.g. perception sanity
        // check failed) BEFORE burning quota — its stderr says exactly why.
        let cause = format!(
            "judge adapter exit {}: {}",
    ...
    match serde_json::from_str::<Value>(&text) {
        ...
        Err(e) => {
            let cause = format!(
                "judge adapter emitted unparseable envelope JSON ({e}); stderr: {}",
                stderr_tail(400)
            );
            JudgeRun {
                envelope: synth_judge_envelope("error", cause.clone()),
