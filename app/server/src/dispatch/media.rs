...
pub(crate) async fn media_import(
    ...
    // Kick the chain job. Result contract (schema/verbs.json, asserted by
    // e2e.sh): {asset_id, job_id}; the op record rides along for the UI.
    ...
    // the inverse-payload contract: route through op_for_result for consistency — import ops carry no
    // inverse (non-undoable), so this is a no-op for size but keeps one shape.
    let mut result = json!({"asset_id": asset_id, "job_id": job, "op": op_for_result(&op, wants_inverse(&args))});
...
pub(super) fn spawn_import_chain(
    ...
    jobs.spawn(&job_id, async move {
        ...
        // first import BECOMES the timeline — end-to-end test contract's e2e flow and the
        // skill workflow go straight from import to transcript editing (the
...
/// dependent verbs (captions / cut_words / remove_fillers) and e2e wait on this
...
fn spawn_enrich_chain(
    ...
    jobs.spawn_limited(&job_id, "analysis", ANALYSIS_MAX_RUNNING, async move {
        ...
        let (st_cb, jid_cb) = (state.clone(), jid.clone());
        let transcript_ok = match run_blocking("media.transcribe", move || {
            ...
            cut_perception::transcribe_progress(&s, &rd, &aid, &h, None, Some(&cb))
        })
        .await
        {
            Ok(_) => {
        ...
        let perception_ok = match run_blocking("media.perception", move || {
            let cb = move |frac: f32, label: &str| {
            ...
            cut_perception::run_instruments_progress(&s, &rd, &aid, &h, set, None, Some(&cb))
        })
        .await
        {
            Ok(_) => {
...
pub(super) async fn media_transcribe(
    ...
    jobs.spawn_limited(&job_id, "analysis", ANALYSIS_MAX_RUNNING, async move {
        ...
        let r = run_blocking("media.transcribe", move || {
            ...
            cut_perception::transcribe_progress(
                &src,
                &receipts,
                &aid,
                &hash,
...
pub(super) async fn media_perception(
    ...
    jobs.spawn_limited(&job_id, "analysis", ANALYSIS_MAX_RUNNING, async move {
        ...
        let r = run_blocking("media.perception", move || {
            cut_perception::run_instruments(&src, &receipts, &aid, &hash, set, None)
        })
        .await;
        match r {
            Ok(_rep) => {
