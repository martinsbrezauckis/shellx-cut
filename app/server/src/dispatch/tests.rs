...
/// forbids. include_inverse is allowed only for verbs that actually echo an op.
#[tokio::test]
async fn unknown_args_rejected_for_every_verb() {
    ...
    // A real op-echoing mutator still accepts the opt-in.
    let dir = tempfile::tempdir().unwrap();
    let r = dispatch(
        &state,
        "project.create",
    ...
    assert!(
        r.ok,
        "edit.add_marker should allow include_inverse: {:?}",
        r.error
    );
}
...
async fn edit_fade_dispatch_contract() {
    ...
    .await;
    assert_eq!(r.error.unwrap().code, "invalid_args");
    // The fade is undoable like any op (snapshot inverse).
...
/// the inverse-payload contract — the fat `inverse` snapshot is dropped from a mutating-verb
/// RESULT by default (token-cost fix), but the canonical op (ops.jsonl + the
/// op_applied event + the persisted log) keeps it so undo/restore still
/// works; `include_inverse:true` opts the echo back in.
#[test]
fn op_for_result_drops_inverse_by_default() {
    // An op WITH an inverse (a normal timeline mutation).
    let op = OpRecord {
    ...
    // Opt-in: full inverse preserved, no marker.
    let full = op_for_result(&op, true);
    assert!(
        full.get("inverse").is_some(),
        "include_inverse keeps the snapshot"
    );
    assert!(full.get("inverse_omitted").is_none(), "no marker when kept");
    // An op WITHOUT an inverse (import/checkpoint) is untouched — no marker.
...
/// the inverse-payload contract flag parse: only boolean true opts in; the public schema rejects
/// stringly booleans before dispatch and this helper stays strict as well.
#[test]
fn wants_inverse_parsing() {
    assert!(wants_inverse(&json!({"include_inverse": true})));
    assert!(!wants_inverse(&json!({})));
    assert!(!wants_inverse(&json!({"include_inverse": false})));
    assert!(!wants_inverse(&json!({"include_inverse": "true"})));
    assert!(!wants_inverse(&json!({"include_inverse": 1})));
    assert!(!wants_inverse(&json!({"include_inverse": "no"})));
}
...
/// include_inverse:true has nothing extra to echo. (the inverse-payload contract omission
/// machinery in op_for_result stays live for HISTORIC logs whose ops still
/// carry snapshot inverses — exercised by the op_for_result unit test.)
#[tokio::test]
async fn add_marker_result_has_no_inverse_but_restore_works() {
    ...
    assert!(
        op["inverse"].is_null(),
        "new ops carry no snapshot inverse: {op}"
    );
    assert!(
    ...
    // include_inverse:true has nothing extra to echo for a new op — there is
    // no snapshot to include under the recompute model.
    let r = dispatch(
        &state,
        "edit.add_marker",
...
async fn assets_generate_imports_generated_file_without_extra_media_import_args() {
    ...
    // Doctor probes can invoke this PATH stub with closed stdin while tests run in
    // parallel. Reject that empty output path; macOS cp otherwise writes beside cutd.
    std::fs::write(
            &codex,
            format!(
                "#!/bin/sh\nprompt=$(cat)\nout=$(printf '%s' \"$prompt\" | awk 'f && NF {{print; exit}} /EXACTLY this path/ {{f=1}}')\ncase \"$out\" in /*) ;; *) exit 4 ;; esac\nprintf 'run\\n' >> '{}'\nif printf '%s' \"$prompt\" | grep -q 'reference variation'; then printf '%s' \"$prompt\" | grep -q 'Reference 1:' || exit 3; cp '{}' \"$out\"; else cp '{}' \"$out\"; fi\nprintf '{{\"ok\":true,\"path\":\"%s\",\"summary\":\"stub\"}}\\n' \"$out\"\n",
                invocations.display(), variation_fixture.display(), fixture.display()
            ),
        )
        .unwrap();
    make_executable(&codex);
...
async fn assets_generate_reserves_replaces_and_retains_pending_timeline_slots() {
    ...
    std::fs::create_dir_all(&bin).unwrap();
    let codex = bin.join("codex");
    // Keep concurrent doctor probes from treating this generation stub as a
    // version command and copying its fixture into the test process cwd.
    std::fs::write(
        &codex,
        format!(
            "#!/bin/sh\nprompt=$(cat)\nout=$(printf '%s' \"$prompt\" | awk 'f && NF {{print; exit}} /EXACTLY this path/ {{f=1}}')\ncase \"$out\" in /*) ;; *) exit 4 ;; esac\nif printf '%s' \"$prompt\" | grep -q 'cancelled slot'; then sleep 30; fi\nsleep 1\ncp '{}' \"$out\"\nprintf '{{\"ok\":true,\"path\":\"%s\",\"summary\":\"stub\"}}\\n' \"$out\"\n",
            fixture.display(),
        ),
    )
    .unwrap();
    ...
    let new_path = match old_path.as_ref() {
        ...
        None => dir.path().join("bin").into_os_string(),
    };
    std::env::set_var("PATH", new_path);
...
fn atomic_output_write_replaces_late_symlink_instead_of_following_it() {
    ...
    let proj = dir.path().join("p.cutproj");
    std::fs::create_dir_all(proj.join("exports")).unwrap();
    let final_path = fence_output_path(&proj, Some("exports/out.srt"), "exports/out.srt")
...
// -----------------------------------------------------------------------
// verify.judge wiring — FAKE adapter (fast + deterministic; the real
// cli_judge.py + claude CLI path is the #[ignore]d live test below).
// -----------------------------------------------------------------------
...
/// (the attempt is honest history) AND the job FAILS with the stderr
/// tail as the actionable cause.
#[tokio::test]
async fn verify_judge_adapter_crash_fails_job_with_stderr() {
    let _env = JUDGE_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let _agent_env = lock_agent_cli_env();
    let dir = tempfile::tempdir().unwrap();
    let (state, rpath) = judge_fixture(dir.path()).await;
    ...
    std::fs::write(
        &stub,
        "import sys; print('judge exploded: flames', file=sys.stderr); sys.exit(3)\n",
    )
    .unwrap();
...
/// Recursive copy skipping `proxies/` (regenerable, keeps the copy light).
fn copy_project_dir(src: &std::path::Path, dst: &std::path::Path) {
...
/// LIVE integration — an explicitly configured judge adapter + the user's
/// subscription CLI reviewing the existing regression render (~2-3 min, burns
/// subscription quota). Run manually:
///   cargo test -p server verify_judge_live_claude_on_fixture_render -- --ignored --nocapture
/// Needs: CUTD_JUDGE_ADAPTER, python3, its corresponding logged-in CLI, and
/// the regression project at .scratch/regression/canvas-demo.cutproj (override:
/// CUTD_JUDGE_TEST_PROJECT). The project is COPIED into
/// .scratch/judge-wirein-live (repo-local — the claude sandbox denies
/// /tmp reads) so the original receipts stay untouched; the original
/// render file is read read-only via the receipt's absolute output_path.
#[tokio::test]
#[ignore = "live subscription-CLI judge: needs claude login, ~3 min, burns quota"]
async fn verify_judge_live_claude_on_fixture_render() {
    let _env = JUDGE_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let _agent_env = lock_agent_cli_env();
    let adapter = std::env::var("CUTD_JUDGE_ADAPTER")
        .expect("set CUTD_JUDGE_ADAPTER to the live external judge adapter");
    assert!(
        Path::new(&adapter).is_file(),
        "CUTD_JUDGE_ADAPTER does not point to a file: {adapter}"
    );
    let repo = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    let src = std::env::var("CUTD_JUDGE_TEST_PROJECT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| repo.join(".scratch/regression/canvas-demo.cutproj"));
    assert!(
        src.is_dir(),
        "regression project not found at {} — set CUTD_JUDGE_TEST_PROJECT",
        src.display()
    );
    let work = repo.join(".scratch/judge-wirein-live");
    let _ = std::fs::remove_dir_all(&work);
    let dst = work.join("canvas-demo.cutproj");
    copy_project_dir(&src, &dst);
    let state = AppState::new();
    ...
    let res = r.result.unwrap();
    let job_id = res["job_id"].as_str().unwrap().to_string();
    let render_id = res["render_id"].as_str().unwrap().to_string();
    let rec = wait_job(&state, &job_id, 420).await;
    println!(
        "JOB RESULT: {}",
        serde_json::to_string_pretty(&rec.result).unwrap()
    );
    assert_eq!(rec.state, crate::jobs::JobState::Done, "{:?}", rec.error);
    let result = rec.result.unwrap();
    assert_eq!(
        result["status"], "completed",
        "live run must produce a real review (claude CLI logged in?)"
    );
    let receipt: cut_core::RenderReceipt = serde_json::from_str(
        &std::fs::read_to_string(dst.join(format!("receipts/{render_id}.json"))).unwrap(),
    )
    .unwrap();
    let judge = receipt.judge.expect("judge attached to receipt");
    println!(
        "JUDGE REVIEW (post-filtered): {}",
        serde_json::to_string_pretty(&judge["review"]).unwrap()
    );
    assert_eq!(judge["schema"], "shellx-cut/judge-review/1");
    assert_eq!(judge["status"], "completed");
    let verdict = judge["review"]["verdict"].as_str().unwrap();
    assert!(
        ["pass", "fail", "needs_review"].contains(&verdict),
        "schema-valid verdict, got {verdict}"
    );
    assert_eq!(
