...
pub(in crate::dispatch) async fn media_index(
    ...
    if !status.success() {
        return Err(CutError::new(
            ...
            format!("siglip_index.py exited with {status}"),
        ));
    }
...
pub(in crate::dispatch) async fn plugins_list(
    ...
    Ok(VerbResult::ok(
        json!({ "plugins": crate::plugins::list_json() }),
    ))
}
...
pub(in crate::dispatch) async fn plugins_enable(
    ...
    crate::plugins::set_enabled(&a.name, enabled).map_err(|e| {
        CutError::new(
            error_codes::IO,
    ...
    Ok(VerbResult::ok(
        json!({ "name": a.name, "enabled": enabled }),
    ))
}
...
pub(in crate::dispatch) async fn plugins_call(
    ...
    if !crate::plugins::is_enabled(&a.plugin) {
        return Err(CutError::new(
...
/// editing (the headline agent-chat feature). Spawns the user's subscription CLI
/// (claude / codex / grok — all wired; `agent` selects, else the first
/// installed+wired in preference order) with cutd's OWN MCP server wired in as its
/// toolset, so its tool calls ARE editing verbs proxied back to THIS running serve
/// (the same open project the UI shows). The op-log is the receipt — every verb the
/// agent calls is a reversible, attributed op (undoable as a unit).
...
///   not_available (no installed+wired agent) · spawn (resolved but failed to
///   launch) · timeout (the headless-approval/login hang) · blocked (codex
///   cancelled the MCP call at its sandbox/approval gate) · auth (login/expired
///   session — `run X login`) · cli_error (non-zero exit, stderr surfaced) ·
...
pub(in crate::dispatch) async fn agent_chat(
    ...
    // Pick the agent: an explicit request must be installed + chat-wired; else the
    // first installed + wired agent in preference order (claude → codex → grok).
    let Some(agent) = crate::chat::pick_agent(a.agent.as_deref()) else {
        ...
        let reason = if let Some(req) = a.agent.as_deref() {
            if crate::chat::CHAT_AGENTS.contains(&req) {
                ...
                format!(
                    "'{req}' is not a chat-wired agent — claude, codex, and grok are. \
                     Detected agents: {installed:?}."
                )
            }
        } else if installed.is_empty() {
            "no coding-agent CLI was found (looked beyond PATH too: ~/.local/bin, Homebrew, \
             ~/.grok/bin). Install claude (or codex/grok) and sign in to use agent chat."
                .into()
        } else {
            format!(
                "no chat-wired agent is available (claude/codex/grok are wired). Detected: {installed:?}."
            )
        };
        return fail(
            "not_available",
    ...
    // shared discovery file). claude's MCP child inherits this from the process
    // env (set below); codex/grok do NOT inherit it, so build_command injects it
    // into their MCP server config — hence it is resolved BEFORE build_command.
    ...
    // A throwaway workspace for the MCP config (the agent never writes here — it
    // edits only via the verbs).
    let suffix = {
    ...
    let ws = std::env::temp_dir().join(format!("cutd-chat-{}-{suffix}", std::process::id()));
    ...
    // install-dir ladder incl. grok's off-PATH ~/.grok/bin) so an off-PATH grok / a
    // Finder-stripped-PATH .app actually LAUNCHES — spawning the bare name would
    // fail. On-PATH claude/codex resolve to themselves, so behavior is unchanged.
    ...
    // Some agents discover their MCP server from a config FILE in the working dir
    // (grok → ./.grok/config.toml) rather than a CLI flag. Write whatever the
    // command declares before spawning; the parent dirs are created as needed.
    ...
    // Agents that take the prompt via STDIN (claude `-p`, `codex exec -`) keep
    // their args as-is; a prompt-FILE agent (grok `--prompt-file`) gets the prompt
    // written to a temp file with the `__PROMPT_FILE__` placeholder substituted.
    ...
    command
        .current_dir(&ws)
        .env("CUTD_PROXY_ADDR", &proxy_addr)
        .env("CUTD_PROXY_ACTOR", &proxy_actor)
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .kill_on_drop(true);
    // codex records a per-working-dir trust entry (+ session state) into its
    // CODEX_HOME config on EVERY run. Left at the default (~/.codex), each chat
    // turn would append a dead `[projects."<temp ws>"]` entry to the USER's
    // ~/.codex/config.toml forever. Isolate CODEX_HOME to the throwaway ws (with
    // the real auth.json symlinked, falling back to a copy, so the subscription
    // login still works) so codex's side effects vanish with the temp dir. SAFE
    // DEGRADATION: only engage when a real auth.json exists and is provisioned —
    // otherwise leave CODEX_HOME alone (codex still authenticates the normal way).
    if agent == "codex" {
        let real_auth = std::env::var_os("CODEX_HOME")
            .map(std::path::PathBuf::from)
            .or_else(|| {
                std::env::var_os("HOME").map(|h| std::path::PathBuf::from(h).join(".codex"))
            })
            .or_else(|| {
                std::env::var_os("USERPROFILE").map(|h| std::path::PathBuf::from(h).join(".codex"))
            })
            .map(|home| home.join("auth.json"))
            .filter(|p| p.exists());
        if let Some(real_auth) = real_auth {
            let iso = ws.join("codex-home");
            let iso_auth = iso.join("auth.json");
            if std::fs::create_dir_all(&iso).is_ok() {
                let provisioned = {
                    #[cfg(unix)]
                    {
                        std::os::unix::fs::symlink(&real_auth, &iso_auth).is_ok()
                    }
                    #[cfg(not(unix))]
                    {
                        false
                    }
                } || std::fs::copy(&real_auth, &iso_auth).is_ok();
                if provisioned {
                    command.env("CODEX_HOME", &iso);
                }
            }
        }
    }
    ...
    let mut child = match spawned {
        ...
        Err(e) => {
            ...
            return fail(
                "spawn",
                format!(
                    "could not start the {} CLI ({e}) — it resolved but failed to launch. \
                     Check it is installed and executable.",
                    cmd.cmd
                ),
                Some(agent.into()),
                vec![],
    ...
    if cmd.via_stdin {
        if let Some(mut stdin) = child.stdin.take() {
            ...
            if let Err(e) = stdin.write_all(prompt.as_bytes()).await {
                ...
                return fail(
                    "cli_error",
                    format!("could not write the prompt to the {agent} CLI stdin: {e}"),
                    Some(agent.into()),
                    vec![],
                    None,
                    None,
            ...
            if let Err(e) = stdin.shutdown().await {
                ...
                return fail(
                    "cli_error",
                    format!("could not close the {agent} CLI stdin: {e}"),
                    Some(agent.into()),
                    vec![],
                    None,
                    None,
            ...
            return fail(
                "cli_error",
                format!("the {agent} CLI stdin pipe was unavailable"),
                Some(agent.into()),
                vec![],
                None,
                None,
    ...
    let out = match waited {
        ...
        Ok(Err(e)) => {
            ...
            return fail(
                "cli_error",
                format!("the {} CLI could not be read to completion: {e}", cmd.cmd),
                Some(agent.into()),
                actions,
                None,
                None,
        ...
        Err(_) => {
            // Timed out — the spec's headless-approval case: a non-interactive run
            // has no human to answer a prompt, so a prompting agent just hangs.
            let (actions, review) = agent_chat_turn_review(
                state,
                ops_before,
                &baseline,
...
async fn assets_generate_run(
    ...
    let mut child = command.spawn().map_err(|e| {
        CutError::new(
            error_codes::IO,
            format!("could not start the {} CLI: {e}", cmd.cmd),
            "the generation CLI must be installed + on PATH",
        )
    })?;
    if cmd.via_stdin {
        if let Some(mut stdin) = child.stdin.take() {
            if let Err(e) = stdin.write_all(prompt.as_bytes()).await {
                ...
                return degrade(format!(
                    "could not write the prompt to the {} CLI stdin: {e}",
                    cmd.cmd
                ));
            }
            if let Err(e) = stdin.shutdown().await {
                ...
                return degrade(format!("could not close the {} CLI stdin: {e}", cmd.cmd));
            }
        } else {
            let _ = child.kill().await;
            if let Some(pf) = &prompt_file {
            ...
            return degrade(format!("the {} CLI stdin pipe was unavailable", cmd.cmd));
        }
    }
    let waited = tokio::time::timeout(timeout, child.wait_with_output()).await;
    if let Some(pf) = &prompt_file {
    ...
    let out = match waited {
        Ok(Ok(o)) => o,
        Ok(Err(e)) => return degrade(format!("the {} CLI errored: {e}", cmd.cmd)),
        Err(_) => {
            return degrade(format!(
...
pub(in crate::dispatch) async fn assets_fetch(
    ...
    Ok(VerbResult::ok_with_ops(
        json!({
            ...
            "op": op_for_result(&op, wants_inverse(&args)),
        }),
        vec![op_id],
    ))
}
