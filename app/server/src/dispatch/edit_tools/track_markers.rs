...
/// - mode:"tip" (DEFAULT) — apply the op's recorded full-timeline inverse.
///   TIP-ONLY: a non-tip target returns a guardrail error (the inverse is a
///   whole-timeline snapshot, so it would discard later ops).
...
pub(in crate::dispatch) async fn edit_restore(
    ...
    let _a: Args = parse_args(args.clone())?;
    // Core's apply handles BOTH modes end-to-end: tip applies the recorded
    // snapshot inverse; rebase runs the dependency gate + id-pinned skip-replay
