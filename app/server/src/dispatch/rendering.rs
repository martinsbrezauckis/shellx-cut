...
pub(super) struct PublishArgs {
    ...
    /// render.final directly. Without it, silent screen-demo publishes receive
    /// caption/loudness failures from the inapplicable default battery.
...
pub(super) async fn render_final(
    ...
    jobs.spawn_limited(&job_id, "render", RENDER_MAX_RUNNING, async move {
        ...
        let rendered = run_blocking("render.final", move || {
            ...
            owned::run_render(render_cancellation, || {
        ...
        let output = match rendered {
            ...
            Err(e) => {
                ...
                if let Some(reason) = cancellation.reason() {
                    return st.jobs.cancel_from_worker(&jid, reason);
                }
        ...
        let facts = run_blocking("verify.checks (output perception)", move || {
            owned::output_facts(sidecar_cancellation, o2, rd, rid, h, dur)
        })
        .await;
        if let Some(reason) = cancellation.reason() {
            return st.jobs.cancel_from_worker(&jid, reason);
...
pub(super) async fn render_direct(
    ...
    jobs.spawn_limited(&job_id, "render", RENDER_MAX_RUNNING, async move {
        ...
        let sheet = run_blocking("direct.contact", move || {
            owned::contact_sheet(sidecar_cancellation, t3, csd, pr)
        })
        .await;
...
pub(super) async fn render_qc(
    ...
    jobs.spawn_limited(&job_id, "analysis", ANALYSIS_MAX_RUNNING, async move {
        ...
        let sheet = run_blocking("reframe.qc", move || {
            owned::qc_sheet(sidecar_cancellation, o2, q2, p2)
        })
        .await;
        if let Some(reason) = cancellation.reason() {
            return st.jobs.cancel_from_worker(&jid, reason);
...
pub(super) async fn render_reframe(
    ...
    jobs.spawn_limited(&job_id, "render", RENDER_MAX_RUNNING, async move {
        ...
        let track = run_blocking("reframe.subject", move || {
            owned::subject_track(sidecar_cancellation, t3, h, preset_c, dir_c)
        })
        .await;
        if let Some(reason) = cancellation.reason() {
            let _ = std::fs::remove_file(&temp);
        ...
        let reframed = run_blocking("reframe.crop", move || {
            owned::run_render(crop_cancellation, || {
...
pub(super) async fn render_bundle(
    ...
    jobs.spawn_limited(&job_id, "render", RENDER_MAX_RUNNING, async move {
        ...
        for (i, (aspect, (w, h))) in dims.iter().enumerate() {
            ...
            let out_path = match fence_project_output_path(&dir, None, &rel, OutputPathPolicy::MP4)
            {
                Ok(p) => p,
                Err(e) => return st.jobs.fail(&jid, e),
            };
            ...
            let facts = run_blocking("render.bundle (perception)", move || {
                owned::output_facts(sidecar_cancellation, o2, rd, rid, hh, dur)
            })
            .await;
            if let Some(reason) = cancellation.reason() {
                return st.jobs.cancel_from_worker(&jid, reason);
...
struct AbortDispatchTask(tokio::task::JoinHandle<VerbResult>);

impl Drop for AbortDispatchTask {
    fn drop(&mut self) {
        self.0.abort();
    }
}

/// Schedule recursive internal verb dispatch from a fresh task stack.
///
/// Storyboard, recipe, render-queue, and autopilot flows can dispatch another
/// verb while their own large async handler is being polled. Keeping every
/// nested poll on one task stack makes correctness depend on OS thread-stack
/// headroom in debug builds. The owned boxed future also breaks the recursive
/// `Send` type cycle. Aborting the child when its waiter is cancelled preserves
/// structured cancellation instead of detaching a mutation.
pub(crate) fn dispatch_send<'a>(
    ...
    Box::pin(async move {
        ...
        match (&mut task.0).await {
            Ok(result) => result,
            Err(error) => VerbResult::err(CutError::new(
                error_codes::JOB_FAILED,
                "nested verb dispatch task failed",
                error.to_string(),
            )),
        }
    })
