...
use cut_core::{
    diff, rebuild_from_log, timeline_snapshot, Actor, ActorKind, Asset, OpRecord, OpStatus,
    ProjectStore,
};
use serde_json::json;
...
fn restore_of_non_tip_op_is_refused() {
    ...
    // Tip restore still works; trailing non-timeline ops (checkpoint) do not block.
...
/// Pre-recompute journals used a full timeline inverse on the original op and
/// wrote an `edit.restore` without a materialized result timeline. Keep that
/// narrow replay path readable while every fresh record remains inverse-free.
#[test]
fn legacy_snapshot_inverse_restore_replays() {
    let d = tempfile::tempdir().unwrap();
    let mut store = ProjectStore::create(d.path(), "legacy", None).unwrap();
    let before = store.project.clone();
    let mut original = store
        .apply(
            "edit.add_marker",
            json!({"at_ms": 250, "label": "legacy marker"}),
            actor(),
            None,
        )
        .unwrap();
    original.inverse = Some(timeline_snapshot(&before));
    let restore = OpRecord {
    ...
    let replayed = rebuild_from_log(&[original, restore]).expect("legacy restore replays");
    assert!(
        replayed.markers.is_empty(),
        "legacy inverse restores the pre-marker timeline"
    );
}
...
/// O(N²). Proven by: (a) no fresh op record carries the legacy snapshot payload,
/// and (b) the
...
fn recompute_keeps_oplog_linear() {
    ...
    // legacy snapshot payload (~O(N) bytes/op → O(N²) total ops.jsonl).
    for k in 0..60u64 {
        s.apply(
            "edit.insert",
    ...
    // (a) No fresh op carries the legacy timeline snapshot signature.
    for l in &lines {
        assert!(
            !l.contains("edit._set_timeline"),
            "no fresh op may carry a legacy timeline snapshot under recompute: {}",
            &l[..l.len().min(120)]
        );
    }
