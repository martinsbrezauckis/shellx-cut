...
/// curve; `segments` the sampling granularity (both recorded on the op so replay
/// is independent of any default).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct SpeedRamp {
    ...
    /// (2–120, resolved + clamped at verb time so replay never depends on the
    /// default).
    pub segments: usize,
}
...
/// integer partition — no rounding gap or overlap), and each sub-segment's
/// timeline duration is `src_off_to_tl(src_span, speed)` (the same rounding the
/// constant-speed path uses), so summing the durations gives the realized
/// timeline length and the sub-segments lay onto the timeline byte-for-byte the
/// way `edl_from_project` places them. SHARED by `Clip::timeline_duration_ms`
/// (sum) and `edl_from_project` (emit) so the cursor math and the rendered
/// segments can never disagree. Empty source slices are dropped; non-empty
/// slices that round to zero timeline duration merge their source span into an
/// adjacent emitted segment so source coverage stays contiguous.
pub fn speed_ramp_segments(src_in: u64, src_out: u64, ramp: &SpeedRamp) -> Vec<RampSeg> {
    ...
    let sd = src_dur as u128;
    let mut out: Vec<RampSeg> = Vec::new();
    let mut pending_src_in: Option<u64> = None;
    for i in 0..n {
        // Exact tiling: slice i spans source offsets [i*sd/n, (i+1)*sd/n).
        let a_off = (i * sd / n) as u64;
        let b_off = ((i + 1) * sd / n) as u64;
        if b_off <= a_off {
            continue; // empty slice (n > src_dur) — skip
        }
        let mid = (a_off + b_off) / 2;
        let speed = speed_ramp_factor_at(ramp, mid);
        let raw_seg_src_in = src_in + a_off;
        let seg_src_in = pending_src_in.take().unwrap_or(raw_seg_src_in);
        let seg_src_out = src_in + b_off;
        let dur_ms = src_off_to_tl(b_off - a_off, speed);
        if dur_ms == 0 {
            if let Some(prev) = out.last_mut() {
                prev.src_out = seg_src_out;
            } else {
                pending_src_in = Some(seg_src_in);
            }
            continue; // a sub-segment must occupy at least 1 ms of timeline
        }
        out.push(RampSeg {
            src_in: seg_src_in,
            src_out: seg_src_out,
