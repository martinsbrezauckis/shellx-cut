...
pub struct ProjectStore {
    ...
    /// history is IN-MEMORY only — never persisted — so redo resets on reopen
    /// (the standard editor convention; on open it is reconstructed from the
    /// log with the cursor at the tip and redo empty). The append-only op-log
    /// stays the immutable record; every undo/redo is itself logged as a
    /// `project.undo` / `project.redo` nav op. See [`Self::commit`],
    /// [`Self::undo`], [`Self::redo`].
...
fn validate_project_name(name: &str) -> Result<(), CutError> {
    ...
    if invalid {
        return Err(CutError::new(
            codes::INVALID_ARGS,
            "project name must be a plain folder name",
            "use letters, numbers, spaces, '-' or '_' without path separators",
        ));
    }
    Ok(())
}
...
impl ProjectStore {
    ...
    pub fn open(dir: &Path) -> Result<Self, CutError> {
        ...
        let ops = log.read_all()?;
        let mut rebuilt = rebuild_from_log(&ops)?;
        reconcile_derived_assets(&mut rebuilt, dir);
        let project = match cached {
            Some(p) if cache_matches_log(&p, &rebuilt) => p,
            _ => rebuilt,
        };
        ...
        // refuse to open.
        // Reconstruct the linear undo history from the log: element 0 = the
        // project.create baseline, then every forward timeline edit in order.
        // The cursor sits at the tip and redo is empty (the in-memory redo
        // branch does not survive a reopen — standard editor convention;
        // documented on the struct + module header). Best-effort: if the last
        // pre-close action was an undo, replay reproduces that undone timeline
        // while the cursor reconstructs at the tip — a benign mismatch (the
        // first undo after such a reopen is a no-op step), never a corruption.
        let undo_history = Self::history_from_log(&log, &project.active_sequence)?;
        let undo_pos = undo_history.len().saturating_sub(1);
        Ok(Self {
    ...
    /// Rebuild the linear undo history (op ids) from a log: element 0 is the
    /// first op (the `project.create` baseline), followed by every forward
    /// timeline edit in order. Shared by `open` (and reusable by any future
    /// reload path). See [`Self::is_history_edit`].
    fn history_from_log(log: &OpLog, active_sequence: &str) -> Result<Vec<String>, CutError> {
        let ops = log.read_all()?;
        ...
        if let Some(first) = ops.first() {
            ...
            histories.insert(current.clone(), vec![first.op_id.clone()]);
        }
        // Consecutive forward edits that share a group_id collapse to
        // ONE history entry pointing at the group's LATEST op (so undo steps over
        // the whole linked action). `last_group` resets across any non-grouped op,
        // so two adjacent groups (or a group then a solo edit) never merge.
        let mut last_group: Option<String> = None;
        for o in &ops {
            if o.verb == "project.sequence_create" {
                let id = o
                    .effects
                    .iter()
                    .find_map(|effect| effect.detail.get("sequence"))
                    ...
                    .ok_or_else(|| {
                        CutError::new(
                            codes::INVALID_ARGS,
                            "sequence-create op is missing its recorded sequence id",
                            format!("op '{}' cannot seed sequence history", o.op_id),
                        )
                    })?;
                current = id.to_string();
                histories.insert(current.clone(), vec![o.op_id.clone()]);
                last_group = None;
                continue;
            }
            if o.verb == "project.sequence_switch" {
                let id = o.args.get("id").and_then(Value::as_str).ok_or_else(|| {
                    CutError::new(
                        codes::INVALID_ARGS,
                        "sequence-switch op is missing id",
                        format!("op '{}' cannot reconstruct sequence history", o.op_id),
                    )
                })?;
                current = id.to_string();
            ...
            if !Self::is_history_edit(o) {
                continue;
            }
            let hist = histories.entry(current.clone()).or_default();
            let g = o.group_id();
            let merge = g.is_some() && g == last_group.as_deref();
            if merge {
                if let Some(last) = hist.last_mut() {
                    *last = o.op_id.clone();
                }
            } else {
                hist.push(o.op_id.clone());
            }
            last_group = g.map(str::to_string);
        }
        Ok(histories.remove(active_sequence).unwrap_or_default())
    }

    /// True when `op` is a FORWARD timeline edit that should advance the linear
    /// undo cursor: it changes the timeline (tracks/markers/caption_styles) AND
    /// is none of —
    ///   - `project.create` — the baseline, seeded as history element 0;
    ///   - `project.format` — a settings-only change (width/height/fps); the
    ///     cursor restores the timeline, not settings, so format stays off it;
    ///   - `project.revert` / `project.undo` / `project.redo` — history
    ///     NAVIGATION ops that move the cursor explicitly and must never be
    ///     pushed as new edits (else undo/redo would interfere with itself).
    /// A rail `edit.restore` (tip or rebase) IS a forward edit — it changes the
    /// timeline deliberately, and undoing it (back to the pre-restore state) is
    /// the correct behavior — so it is intentionally NOT excluded here.
    fn is_history_edit(op: &OpRecord) -> bool {
        op.mutates_timeline()
            && !matches!(
                op.verb.as_str(),
                "project.create"
                    | "project.format"
                    // settings-only color management change (working/output color
                    // space); like project.format the cursor restores the timeline,
                    // not settings, so it stays off the undo cursor.
                    | "project.color"
                    | "project.revert"
                    | "project.undo"
                    | "project.redo"
    ...
    fn record_history_commit(&mut self, op: &OpRecord) {
        // Linear undo-history bookkeeping: a new forward edit truncates any
        // redo future (the standard "a fresh edit kills the redo branch") and
        // becomes the new cursor tip. Navigation ops (undo/redo/revert), the
        // create baseline, and settings-only format changes are excluded — they
        // move the cursor explicitly (or not at all), never push. In-memory
        // only; the on-disk log is untouched by this.
        if Self::is_history_edit(op) {
            self.undo_history.truncate(self.undo_pos + 1);
            // Grouping: an edit carrying the SAME group tag as the
            // current tip EXTENDS that entry (the group's latest op becomes the
            // undo target) instead of pushing a new step — so a linked A/V paste
            // / linked delete is ONE Ctrl+Z. A fresh tag (or None) pushes a new
            // entry. `tip_group` tracks the tip entry's tag.
            let g = op.group_id();
            let merge =
                g.is_some() && g == self.tip_group.as_deref() && !self.undo_history.is_empty();
            if merge {
                if let Some(last) = self.undo_history.last_mut() {
                    *last = op.op_id.clone();
                }
                // undo_pos already at the tip; leave it.
            } else {
                self.undo_history.push(op.op_id.clone());
                self.undo_pos = self.undo_history.len() - 1;
            }
            self.tip_group = g.map(str::to_string);
        }
    }

    /// Append an op to the log and publish the staged project cache. The staged
    /// project is not swapped into memory until after the append succeeds, so a
    /// failed log write can never leave live/cache state ahead of ops.jsonl.
    pub fn commit_staged(&mut self, mut next: Project, op: &OpRecord) -> Result<(), CutError> {
        next.sync_active_sequence();
        let encoded = serde_json::to_string_pretty(&next)?;
        self.log.append(op)?;
        let cache_result = self.write_project_json(&encoded);
        self.project = next;
        if matches!(
            op.verb.as_str(),
            "project.sequence_create" | "project.sequence_switch"
        ) {
            self.undo_history = Self::history_from_log(&self.log, &self.project.active_sequence)?;
            self.undo_pos = self.undo_history.len().saturating_sub(1);
            self.tip_group = None;
        } else {
            self.record_history_commit(op);
        }
        cache_result
    }

    /// Append an op to the log and save the current cache. Prefer
    /// [`Self::commit_staged`] for mutators so append failures cannot leak
    ...
    /// error can never half-mutate the cache), capture the snapshot inverse,
    ...
    pub fn apply(
        ...
        let effects = if verb == "edit.restore" {
            ...
            let later: Vec<(&str, &str)> = all[pos + 1..]
                ...
                .filter(|(o, sequence)| {
                    o.mutates_timeline() && *sequence == &self.project.active_sequence
                })
                .map(|(o, _)| (o.op_id.as_str(), o.verb.as_str()))
                .collect();
            ...
            let pre = rebuild_from_log(&all[..pos])?;
    ...
    /// each step transactionally, captures the snapshot inverse, and records
    ...
    /// them can make renders and proxies faster on heavy footage
    /// ask. Like `rename`, this is project METADATA, not a timeline
    /// edit (inverse: None) — the new settings live in project.json (saved by
    /// commit), so replay starts from them; the op is recorded for audit. Any of
    /// width/height/fps may be None to leave it unchanged; values are validated.
    pub fn set_format(
        &mut self,
        width: Option<u32>,
        height: Option<u32>,
    ...
    pub fn revert(&mut self, to: &str, actor: Actor) -> Result<Vec<String>, CutError> {
        ...
        let all = self.log.read_all()?;
        if let Some(assets) = motion_assets_for_checkpoint(&all, to)? {
            extra.insert(
                "removed_assets".into(),
        ...
        // pushes either way.
        if let Some(p) = self.undo_history.iter().position(|id| id == &target) {
            self.undo_pos = p;
        }
        Ok(vec![rec.op_id])
    }
    ...
    fn set_timeline_as_of(
        ...
        let rebuilt = rebuild_from_log(&all[..=idx])?;
    ...
    pub fn undo(&mut self, actor: Actor) -> Result<OpRecord, CutError> {
        ...
        let removed_assets = motion_assets_for_op(&all, &leaving)?;
        let removed_checkpoint = motion_checkpoint_for_op(&all, &leaving)?;
    ...
    pub fn redo(&mut self, actor: Actor) -> Result<OpRecord, CutError> {
        ...
        let target = self.undo_history[target_pos].clone();
        let all = self.log.read_all()?;
        let restored_assets = motion_assets_for_op(&all, &target)?;
        let restored_checkpoint = motion_checkpoint_for_op(&all, &target)?;
        let mut extra = serde_json::Map::new();
        extra.insert("to_op".into(), json!(target));
        extra.insert("cursor".into(), json!(target_pos));
    ...
    /// the append-only operation-log contract / timeline/op-log contract); its inverse is a normal pre-op snapshot, so the
    /// rebase is itself tip-undoable.
    ///
    /// SAFETY (the prime rule: a half-working rebase is worse than none):
    /// 1. The target must be a timeline-mutating op (have an inverse). Refuses
    ...
    pub fn rebase_out(
        ...
        let rebased_over: Vec<String> = all[pos + 1..]
            .iter()
            .filter(|o| o.mutates_timeline())
            .map(|o| o.op_id.clone())
            .collect();

        // APPEND the rebase op. The recomputed timeline rides in
        // `rebase_new_timeline` so replay reproduces it without re-running the
        // dependency analysis; the op carries no snapshot inverse (recompute-by-
        // replay), and is itself tip-undoable (a tip restore recomputes the
...
pub fn apply_edit_verb_pinned(
    ...
    match verb {
        ...
        "edit.speed_ramp" => {
            ...
            // resolves `segments` (default + clamp) BEFORE commit, so the op log
            // stores the explicit curve + granularity and replay reproduces it
            // verbatim. An EMPTY points list clears the ramp. preserve_pitch rides
            ...
            let a: A = serde_json::from_value(args.clone())?;
            edit::speed_ramp(project, &a.clip, a.points, a.segments)
        }
        "edit.move" => {
            #[derive(serde::Deserialize)]
            struct A {
...
pub fn apply_record(
    ...
    match op.verb.as_str() {
        "project.create" => {
            ...
            validate_project_name(name).map_err(|_| {
                replay_corrupt(
                    op,
        ...
        "project.format" => {
            ...
            // effect, so replay is a direct settings assignment — no inverse, no
            // timeline mutation. WITHOUT this arm any project that ever changed
            // format was unreplayable: recompute-by-replay undo, rebuild_from_log
            // cold-reopen, and rebase across the format op all hit the
            // "not a core verb" escape and failed (caught by verb-walkthrough
            //. The snapshot is authoritative — clamping already
...
/// reopen (replay reproduces the undone timeline; redo resets — by convention).
#[cfg(test)]
mod undo_redo_tests {
    use super::*;
    ...
    fn smart_bins_persist_replay_and_are_offcursor() {
        ...
        assert!(!replaced);
        assert!(!op.mutates_timeline(), "bin_save is a metadata op");
    ...
    fn caption_style_presets_persist_and_replay() {
        ...
        let (replaced, op) = s
            ...
            .unwrap();
        assert!(!replaced);
        assert!(!op.mutates_timeline(), "save_style is a metadata op");
    ...
    fn set_color_persists_and_replays() {
        ...
        assert!(!ProjectStore::is_history_edit(last));
    }
    ...
    fn set_brand_persists_replays_and_clears_off_cursor() {
        ...
        let op = store
            ...
            .unwrap();
        assert!(!ProjectStore::is_history_edit(&op));
        assert_eq!(
        ...
        assert!(!ProjectStore::is_history_edit(&clear));
    ...
    /// Across a close+reopen: replay reproduces the undone timeline (the log is
    /// the truth), the cursor reconstructs at the tip and redo is empty
    /// (in-memory only) — the documented convention.
    #[test]
    fn undo_survives_reopen_via_replay() {
        ...
        // History reconstructed: baseline + 2 forward edits; cursor at tip,
        // redo empty.
        assert!(!s2.redo_available(), "redo is empty after reopen");
        assert!(
            s2.undo_available(),
