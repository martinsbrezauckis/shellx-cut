...
//! `project.json` (store::rebuild_from_log). Today `edit.restore` is TIP-ONLY:
//! inverses are FULL-TIMELINE snapshots, so restoring a non-tip op rolls the
//! whole timeline back to its pre-op state — silently discarding every later
//! edit. The store guardrail refuses that and names the blockers (store::apply).
//!
...
//!
//!   1. Inverses are full-timeline snapshots, not per-op forward inverses — so
//!      we cannot just "apply N's inverse"; we must REPLAY the log with N's
//!      effects omitted.
...
//! APPENDS a fresh op whose recorded inverse is a normal full-timeline snapshot
//! (so the rebase is itself tip-undoable). Replay of a log CONTAINING a rebase
//! op reproduces its effect from the recorded skip-set WITHOUT re-running the
//! dependency analysis — determinism over the analysis is not required, only
//! determinism over the recorded result.
//!
...
/// Ops without inverses (project.create, project.checkpoint, media.import) are
/// not timeline mutations and are neither rebase targets nor dependents — but
