...
//!
//! Undo model: every mutating op's inverse is a
//! FULL timeline snapshot taken before the op (`edit._set_timeline`
//! {tracks, markers}). `restore(op)` applies that snapshot — which means
//! restoring an op also discards any LATER timeline edits. The review-rail
//! skim flow rejects ops newest-first so this is correct in practice;
//! surgical per-verb inverses remain an explicit compatibility refinement.
//!
...
/// variable-speed curve. `points` (≥ 2, source-offset/factor; see
/// [`crate::types::SpeedRampPoint`]) describe a piecewise-linear speed curve over
/// the clip's source window; `segments` is the sampling granularity (resolved +
/// clamped by the dispatch layer, recorded so replay is default-independent). An
/// EMPTY `points` list CLEARS the ramp (back to constant speed). The realized
/// timeline length is the integral of (1/speed) over the source = the sum of the
/// expanded sub-segment durations (`speed_ramp_segments`), so later clips ripple
/// automatically (the cursor reads `timeline_duration_ms`).
///
...
pub fn speed_ramp(
    ...
    let (track_id, idx) = project
        ...
        .map(|(t, i)| (t.to_string(), i))
        .ok_or_else(|| clip_not_found(clip_id))?;
    // Capture the project fps BEFORE the mutable track borrow — needed for the
    // frame-aware segment clamp below (a pure function of project state + args, so
    // replay re-derives the same clamped count deterministically).
    ...
    // FRAME-AWARE clamp: keep every constant-speed sub-segment ≥ MIN_FRAMES_PER_SUBSEG
    // OUTPUT frames at the ramp's FASTEST point, so per-segment whole-frame video
    // quantisation stays sub-frame and never drifts from the sample-accurate audio
    // atempo (a 48-way split of a short/fast clip otherwise renders sub-frame video
    // segments the concat drops → measured A/V desync). frame_cap = src_dur·fps /
    // (max_factor · MIN_FRAMES · 1000); the result is recorded on the ramp so replay
    // is deterministic.
    let src_dur = c.src_out_ms.saturating_sub(c.src_in_ms);
    let max_factor = points.iter().map(|p| p.factor).fold(1.0_f64, f64::max);
    ...
    let ramp = crate::types::SpeedRamp { points, segments };
...
/// drop content). The undo inverse is the usual full-timeline snapshot, so a
/// removed track and its clips come back on undo.
pub fn remove_track(
    project: &mut Project,
    track_id: &str,
    force: bool,
...
/// Undo a previous op by applying its recorded inverse (timeline/op-log contract rules:
/// this APPENDS a new op; the old record is never rewritten). The caller
/// passes the original record; this function applies the inverse state
/// change and returns the effects for the new `edit.restore` record.
///
/// Timeline inverse model: `edit._set_timeline{tracks,markers}` — the snapshot
/// taken before the original op (see module header for the tradeoff).
pub fn restore(
    project: &mut Project,
