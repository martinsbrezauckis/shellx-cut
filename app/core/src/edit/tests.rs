...
fn ripple_delete_refuses_speed_ramped_clip_before_mutation() {
    let mut p = fixture();
    speed_ramp(&mut p, "c1", ramp_pts(), 24, None, None).expect("set ramp");
...
fn speed_ramp_sets_guards_and_clears() {
    ...
    speed_ramp(&mut p, "c1", vec![], 24, None, None).expect("clear ramp");
...
fn speed_ramp_clamps_segments_to_frame_floor() {
    ...
    speed_ramp(&mut p, "c1", ramp_pts(), 80, None, None).expect("set ramp");
    let c1 = match &p.track("v1").unwrap().clips[0] {
        Clip::Media(c) => c.clone(),
        _ => unreachable!(),
    };
    ...
    speed_ramp(&mut p, "c2", ramp_pts(), 6, None, None).expect("set ramp c2");
...
fn speed_ramp_refuses_incompatible_clip() {
    ...
    assert_eq!(
        speed_ramp(&mut p, "c2", ramp_pts(), 24, None, None)
            .unwrap_err()
            .code,
        "invalid_args"
    );
    ...
    assert_eq!(
        speed_ramp(&mut p2, "c2", ramp_pts(), 24, None, None)
            .unwrap_err()
            .code,
        "invalid_args"
    );
