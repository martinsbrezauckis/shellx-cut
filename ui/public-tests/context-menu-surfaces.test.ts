...
assert.match(timelineContextCoverage, /async function waitForTrackLockDom/, 'native context-menu coverage owns explicit track-lock DOM synchronization')
assert.match(timelineContextCoverage, /await waitForTrackLockDom\(page, baseVideo, true\)[\s\S]*openLockedClip/, 'locked-track coverage waits for the locked DOM branch before opening its menu')
assert.match(timelineContextCoverage, /await waitForTrackLockDom\(page, baseVideo, false\)[\s\S]*openTrack/, 'track coverage waits for the unlocked DOM branch before reopening its menu')
assert.match(timelineContextCoverage, /WebKitGTK can acknowledge[\s\S]*await target\.click\(options\)[\s\S]*one bounded retry/, 'Linux WebDriver may retry one dropped native context gesture without synthesizing a DOM event')
assert.match(timelineContextCoverage, /native context menu did not open after one bounded retry/, 'a second native context delivery failure remains release-blocking')
