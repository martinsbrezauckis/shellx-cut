...
//
// Red-proof for two 0.6.106 hotfix defects (2026-08-06 demo shoot, finding 4 +
// the "grade sliders quantize to 0.05" roadmap item):
//
...
//    level — the same pattern lib.test.ts already uses for the gradeSeedKey
//    resync contract: the seed effect must not touch result/err, and a
...
// ---- 1. slider precision: 0.01 step on the eq sliders, 100 K on temp -------
{
  ...
  // rig lanes: fullCoverageGradeActions + interaction-verify read them numerically).
...
{
  // Locate the slider-seed effect by its dep anchor (same anchor lib.test.ts pins).
