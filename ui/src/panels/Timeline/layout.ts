 ...
 *    edit.fit_to_fill (defined in app/core/src/edit.rs + trim_edit.rs).
 ...
 * (engine not_found). */
/**
 * The UI-facing semantic class of a laid timeline item.
 *
 * `TrackKind` deliberately remains the engine's small video/audio/caption
 * storage union. This projection carries the extra UI ownership needed for
 * generated overlays and stills without changing the replayable project
 * format: title/shape annotations come from `project.state`, and title-track
 * ownership remains the compatibility fallback for kinetic overlays.
 */
export type TimelineContentClass = 'video' | 'audio' | 'still' | 'title' | 'shape' | 'caption' | 'gap'

/** Classes backed by an editable timeline media clip rather than a caption/gap. */
export function isMediaContentClass(contentClass: TimelineContentClass): boolean {
  return contentClass === 'video' || contentClass === 'audio' || contentClass === 'still' ||
...
export interface LaidItem {
  ...
  /** Semantic UI ownership, projected once while materializing project clips. */
...
/**
 * Classify a persisted media clip for UI behavior at the project-layout
 * boundary. Do not rediscover title/shape ownership from DOM selectors or
 * menu-local track-id checks: every timeline consumer receives this same
 * projection through `LaidItem.contentClass`.
 */
function mediaContentClass(track: Track, clip: MediaClip, imageAssets?: Set<string>): TimelineContentClass {
  if (track.kind === 'audio') return 'audio'
  if (typeof clip.shape_kind === 'string') return 'shape'
  if (typeof clip.title_text === 'string') return 'title'
  // Kinetic captions and older title projects may not carry the transient
  // per-clip decoration. The engine owns `title*` as their generated-overlay
  // track namespace, so retain it as the backwards-compatible source fact.
  if (track.id.startsWith('title')) return 'title'
  if (imageAssets?.has(clip.asset)) return 'still'
  return 'video'
}
  ...
  track.clips.forEach((c, i) => {
    ...
    if (gap) {
      ...
      items.push({
        ...
        contentClass: mediaContentClass(track, clip, imageAssets),
        trackId: track.id,
        startMs,
...
export function trackSeams(items: LaidItem[]): Seam[] {
  ...
  for (let i = 0; i < items.length - 1; i++) {
    ...
    // edit.crossfade there is not_found. A crossfade ON this seam itself keeps
    // the same editorial at_ms and re-targets the original cut. The handle is
    // DRAWN at the laid end (`laidMs`) so it
