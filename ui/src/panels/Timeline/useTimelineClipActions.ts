...
import { adjacentGapSlot, editorialTransitionAtMs, exactTimelineAudioTarget, isContiguousRun } from './ClipContextMenuModel'
...
export function useTimelineClipActions({ cfg, setActiveSeam }: UseTimelineClipActionsArgs) {
  ...
  const crossfadeAdjacent = useCallback((itemId: string) => {
    const c = cfg.current
    const it = c.allItems.find((i) => i.id === itemId)
    if (!it || !isMediaContentClass(it.contentClass)) return
    // The menu and dispatch share this exact editorial-time resolver. The
    // visible seam is laid/render time; edit.crossfade keys its engine op by
    // the stable cumulative editorial boundary instead.
    const atMs = editorialTransitionAtMs(it, c.allItems)
    if (atMs === null) return
  ...
  /** Audio verbs accept audio tracks only. The shared resolver refuses a
   * near-match, so an independently trimmed sibling can never dispatch a
   * video id to edit.gain/audio.cleanup_voice. */
  const audioHalfOf = useCallback((itemId: string): string | null => {
    const c = cfg.current
    const it = c.allItems.find((i) => i.id === itemId)
    return it ? exactTimelineAudioTarget(it, c.allItems) : null
  }, [cfg])
  ...
  const applyCrossfade = useCallback((seam: Seam, durationMs: number, transition = 'dissolve') => {
    void runUserVerb('edit.crossfade', {
      ...
      // it returns not_found on any seam downstream of an existing crossfade.
      at_ms: seam.atMs,
