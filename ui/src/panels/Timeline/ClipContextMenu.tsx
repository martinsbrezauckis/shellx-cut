...
export default function ClipContextMenu({
  ...
  const videoHasAudio = isVideo && !!it?.asset && allItems.some(
    (a) => a.kind === 'audio' && a.asset === it.asset && a.startMs === it.startMs,
  )
  const hasAudio = isAudio || videoHasAudio
  ...
  const sourceAssets = Object.entries(project?.assets ?? {}).filter(([id, asset]) => {
    ...
    if (isVideo) return mediaKind === 'video' || mediaKind === 'image' || mediaKind === 'other'
    ...
    <>
      ...
      >
        ...
        {showSpeed && (
          <>
            <span className="tl-ctx__sep" aria-hidden="true" />
            <span className="tl-ctx__label" aria-hidden="true">Speed</span>
            <button className="tl-ctx__item" data-cut-ctx="speed-half" role="menuitem"
              title="Slow to half speed"
              onClick={() => { speedItem(menu.itemId, 0.5); onClose() }}>
              <Icon name="speed" size={14} /> ½× speed
            </button>
            <button className="tl-ctx__item" data-cut-ctx="speed-normal" role="menuitem"
              title="Reset to normal speed"
              onClick={() => { speedItem(menu.itemId, 1); onClose() }}>
              <Icon name="speed" size={14} /> 1× (normal)
            </button>
            <button className="tl-ctx__item" data-cut-ctx="speed-double" role="menuitem"
              title="Speed up to twice normal speed"
              onClick={() => { speedItem(menu.itemId, 2); onClose() }}>
              <Icon name="speed" size={14} /> 2× speed
            </button>
            <button className="tl-ctx__item" data-cut-ctx="speed-custom" role="menuitem"
              title="Type a custom speed factor (0.25–4×)"
              onClick={() => {
                const raw = window.prompt('Playback speed factor (0.25–4):', '1')
                onClose()
                const factor = raw === null ? NaN : Number(raw)
                if (Number.isFinite(factor) && factor >= 0.25 && factor <= 4) speedItem(menu.itemId, factor)
              }}>
              <Icon name="speed" size={14} /> Custom speed…
            </button>
            <button className="tl-ctx__item" data-cut-ctx="reverse" role="menuitem"
              title="Play the clip backward"
              onClick={() => { reverseItem(menu.itemId); onClose() }}>
              <Icon name="flip" size={14} /> Reverse
            </button>
            {showFreeze && (
              <button className="tl-ctx__item" data-cut-ctx="freeze" role="menuitem"
        ...
        <button className="tl-ctx__item" data-cut-ctx="add-transition" role="menuitem"
          disabled={!hasAdjacentSeam}
          title={!hasAdjacentSeam ? 'Needs an adjacent clip on the same track' : 'Crossfade the adjacent cut'}
        ...
        {showPrivacy && (
          <>
            <span className="tl-ctx__sep" aria-hidden="true" />
            <span className="tl-ctx__label" aria-hidden="true">Privacy</span>
        ...
        <span className="tl-ctx__sep" aria-hidden="true" />
        <span className="tl-ctx__label" aria-hidden="true">Fade</span>
