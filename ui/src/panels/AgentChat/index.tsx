...
// `agent.chat` verb spawns the user's OWN subscription CLI (claude/codex/grok)
// wired to cutd's MCP server, so the agent's tool calls ARE editing verbs on the
// live project. Request/response per turn (no token streaming): a "working…"
...
// so we re-scan when the menu opens; it's cheap). Each agent shows a 3-state badge
// (ready / needs-login / install) + an INFORMATIONAL posture tag. DEFAULT = claude
// (sandboxed, ready); the choice persists (lib/chatAgentPref) and rides into
...
export default function AgentChat({ project, prefill }: AgentChatProps) {
  ...
  const chooseAgent = useCallback((name: ChatAgentName) => {
    setAgent(name)
    setChatAgent(name)
    setMenuOpen(false)
  }, [])
  ...
  const send = useCallback(async () => {
    ...
    setBusy(true)
    try {
      // Pass the chosen backend so cutd uses the right calling mechanics.
                  ...
                  <div className="chat__agentopt-top">
                    ...
                    {posture && (
                      <span
                        ...
                        title="Security posture this agent runs at, headless — informational, not a restriction"
                      >
                        {posture.text}
                      </span>
                    )}
