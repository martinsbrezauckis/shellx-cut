// panels/Review/mock.ts — deterministic offline demo (active only on `?mock=1`).
...
// local UI review without requiring a running cutd process.
...
class FakeWS {
   ...
   * offline demo; cutd would consume them in a connected session. */
  static sent: string[] = []
...
function handleVerb(name: string, args: Record<string, unknown>): unknown {
  switch (name) {
    ...
    case 'project.health':
      return {
        ...
        result: {
          ...
          project_revision: OPS[OPS.length - 1]?.op_id ?? 'op_000001',
          journal: {
            status: 'verified',
            log_records: OPS.length,
            cache: 'matched',
            snapshot: { status: 'not_present' },
            notices: [],
          },
          media: {
            status: 'ready',
    ...
    case 'screen_record.recovery_status':
      return { ok: true, result: { captures: [], next_cursor: null } }
    case 'media.check':
      return {
        ok: true,
        result: {
    ...
    case 'verify.checks':
      return {
        ...
        error: {
          code: 'not_found',
          message: 'no persisted receipt in offline demo mode',
