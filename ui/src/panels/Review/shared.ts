...
const TRANSCRIPT_CUT_VERBS = new Set([
...
export function activeCutSpans(ops: OpRecord[]): CutSpan[] {
  ...
  for (const op of ops) {
    ...
    if (!TRANSCRIPT_CUT_VERBS.has(op.verb)) continue
    const args = objectFrom(op.args)
    if (args) {
...
// design: inverses are full-timeline snapshots, so only the LATEST
...
// Timeline-mutating classification now lives in lib/ops (mutatesTimeline) — a
// BLOCKLIST mirror of the engine's OpRecord::mutates_timeline(), so new verbs
// (edit.grade, title.add, captions.kinetic, …) are correctly undoable by default
// instead of being silently dropped by a stale allowlist.

/** The newest applied, not-yet-restored INVERSE-BEARING op — the only op
 * edit.restore{mode:"tip"} will accept (the "tip"). null = nothing to undo.
 *
 * `edit.restore` ops COUNT: the engine's tip is the latest op carrying an
 * inverse, and restore ops carry one (store.rs selective-undo guard filters on
 * `inverse.is_some()`). So after any undo/rebase the restore op itself is the
 * tip — undoing it is a redo, which the engine accepts. The rebase-rail walk
