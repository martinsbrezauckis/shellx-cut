...
export default function Record({ project, onClipAdded, onOpenOutputSettings }: RecordProps) {
  ...
  // Defaults OFF SAFETY): the native Windows WASAPI loopback held the render
  // device on a hung capture and muted system audio, so it must be an EXPLICIT opt-in
  // until the teardown is proven non-intrusive on the rig. Linux uses the pulse monitor.
  ...
  const probe = useCallback(async () => {
    ...
    if (r.ok && r.result) {
      const res = r.result as { cards: RecordCard[]; ready: boolean; monitors?: MonitorInfo[]; windows?: WindowInfo[] }
      setCards(res.cards)
      setReady(res.ready)
  ...
  const finalize = useCallback(async (captureId: string, source: string | null) => {
    ...
    try {
      ...
      if (rawMode) {
        ...
        const sr = stop.result as { raw_path?: string; raw_has_mic?: boolean; raw_has_system?: boolean; source?: string; raw_streams?: StudioRawStreams }
      ...
      if (!stop.ok) { setErr(`stop failed: ${stop.error?.message ?? 'error'}`); setPhase('error'); return }
      const sr = stop.result as { source?: string; plan?: string; raw_streams?: StudioRawStreams }
      setLastRawStreams(sr.raw_streams ?? null)
  ...
  // project isn't open / capture isn't ready (matches the button's disabled state).
  const toggle = useCallback(() => {
    ...
    if (phase === 'recording') {
      ...
      if (ready !== false) void start() // start() auto-creates a project if none is open
    }
  }, [phase, project, ready, start, stop])
  ...
  const cardStatus = (c: RecordCard) => (c.status === 'ok' ? 'ok' : c.status === 'degraded' ? 'degraded' : 'missing')
          ...
          <div className="rec__cards" data-cut-rec-cards data-cut-rec-readiness>
            ...
            {ready === false && (
              <p className="rec__not-ready" data-cut-rec-not-ready>
                Screen capture isn’t fully available on this machine — recording needs a desktop session
                (Linux XDG portal / Windows / macOS) with ffmpeg. Core editing still works.
              </p>
            )}
          </div>
            ...
            {phase === 'done' && !rawCapture && lastCapture && (
              <div className="rec__export" data-cut-rec-export>
                ...
                <div className="rec__seg">
                  ...
                  <button type="button" className="rec__export-btn" data-cut-action="record-export" onClick={() => void exportClip()}>
                    Export clip…
                  </button>
                </div>
                {exportNote && <p className="rec__export-note" data-cut-rec-export-note>{exportNote}</p>}
              </div>
            )}
