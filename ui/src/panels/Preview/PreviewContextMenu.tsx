...
export default function PreviewContextMenu({ monitorRef, video, hasProject, playheadMs, onSeek }: PreviewContextMenuProps) {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (event: MouseEvent) => {
      ...
      setMenu({ x: event.clientX, y: event.clientY, video, atMs: Math.max(0, Math.round(playheadMs)) })
    }
    monitor.addEventListener('contextmenu', onContextMenu)
    return () => monitor.removeEventListener('contextmenu', onContextMenu)
  }, [monitorRef, playheadMs, video])
  if (!menu) return null
  ...
  return <ContextMenuFrame x={menu.x} y={menu.y} menuId="data-cut-preview-menu" backdropId="data-cut-preview-ctx-backdrop" onClose={() => setMenu(null)}>
