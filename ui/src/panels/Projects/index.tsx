...
export default function ProjectsPanel({ onReopen, currentName, active }: ProjectsPanelProps) {
  ...
  const projectMenuTarget = projectMenu ? projects.find((project) => project.id === projectMenu.projectId) ?? null : null
	        ...
	        <div className="pj-list">
	          {shown.map((p) => {
                ...
                onContextMenu={(event) => {
                  ...
                  setProjectMenu({ x: event.clientX, y: event.clientY, projectId: p.id })
                }}
	              >
	                <button
	                  type="button"
      ...
      {projectMenu && (
        <ProjectContextMenu
          ...
          project={projectMenuTarget && {
            id: projectMenuTarget.id,
            name: projectMenuTarget.name,
            missing: projectMenuTarget.missing,
            current: currentName != null && projectMenuTarget.name === currentName,
          }}
          busy={busy}
          onReopen={(projectId) => {
            const project = projects.find((candidate) => candidate.id === projectId)
            if (project) void reopen(project)
          }}
          onForget={(projectId) => {
            const project = projects.find((candidate) => candidate.id === projectId)
            if (project) void forget(project)
          }}
          onDelete={(projectId) => {
            const project = projects.find((candidate) => candidate.id === projectId)
            if (project) void del(project)
          }}
