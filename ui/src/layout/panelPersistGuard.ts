...
// restore after a render crash. A panel that terminates the WebView while first
// painting must not be restored automatically on every subsequent launch.
//
...
// interaction) is not caught by the sentinel. The guard covers failures during
// opening/painting. recordPanelRenderFailure() also
 ...
 *  boot: without this, a reload landing inside the arm→confirm window can
 *  blocklist a tab that rendered normally.
 *
