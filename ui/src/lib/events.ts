...
export type CutEvent =
  | {
    type: 'op_applied'
    op: OpRecord
    /** Revision after this durable operation, for gap-aware sync pulls. */
    revision?: string
    /** Revision the sender observed before this durable operation. */
    from_revision?: string
    delta?: { kind: string; count: number }
  }
...
function cutEventFrom(v: unknown): CutEvent | null {
  ...
  switch (obj.type) {
    case 'op_applied': {
      ...
      if (!op) return null
      const revision = stringField(obj, 'revision')
      const fromRevision = stringField(obj, 'from_revision')
      const delta = recordFrom(obj.delta)
      const kind = delta ? stringField(delta, 'kind') : undefined
      const count = delta ? numberField(delta, 'count') : undefined
      return {
        type: 'op_applied',
        op,
        ...(revision ? { revision } : {}),
        ...(fromRevision ? { from_revision: fromRevision } : {}),
        ...(kind && count != null ? { delta: { kind, count } } : {}),
      }
    }
    case 'job_progress': {
...
export class EventsClient {
   ...
   *
   * Reliability contract for the ui.screenshot primitive:
