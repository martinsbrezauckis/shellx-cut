...
export interface VerbArgs {
  ...
  // include_inverse is a deprecated v0.6 compatibility no-op: fresh records
  // use recompute-by-replay and contain no inverse payload.
  ...
  // mode (op-rebase): "tip" (DEFAULT) recomputes the LATEST timeline op's
  // pre-target journal prefix — a deeper
...
export async function fetchProject(): Promise<Project | null> {
  ...
  return r.ok ? (r.result as Project | undefined) ?? null : null
}
