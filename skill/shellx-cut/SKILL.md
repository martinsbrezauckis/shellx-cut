...
> `scripts/coverage-audit.sh`). **`reference.md` is the full 260-verb table —
...
> - **Agent chat (natural-language editing)** — `agent.chat` spawns the user's
>   OWN subscription CLI (`claude`, `codex`, or `grok`) with cutd's MCP server
>   wired in as its toolset, so the agent's tool calls ARE editing verbs on the
>   SAME live project (no model hosted; subscription auth, not a metered key).
>   Every applied verb is a normal reversible op. Claude remains the default
>   preference, but explicit Codex/Grok turns are wired through their own MCP
>   configuration paths and fail honestly when auth or tool approval blocks them.
...
>   as a permission fence over the SAME verb registry. A disabled or
>   out-of-scope call fails closed; this is not a second extension API.
...
>   `studio_event` / `autoedit` / `polish` / `export` (live screen/audio
...
>   asks, and verify `raw_has_system:true` after `screen_record.stop`.
...
- Antigravity CLI: the qualified `agy` 1.1.9 client has no `agy mcp add`
  subcommand. Add
...
the final proof of protocol negotiation, ping, all 260 tools, and same-engine
...
`{type: op_applied | job_progress | render_done | receipt_ready | project_changed | ui_state | doctor_updated, ...}`.
...
- Ops are immutable. To undo, append: `edit.restore {op_id}`. Never try to
  delete or rewrite history. **Default restore is tip-only** (`mode:"tip"`):
  inverses are full-timeline snapshots, so the default restore only undoes the
  LATEST timeline op — reject ops as you review (newest first). To selectively
...
fallback port.
Result excerpts below match the live server (e2e-verified shapes).
